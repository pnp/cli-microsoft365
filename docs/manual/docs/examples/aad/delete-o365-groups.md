...
There are so many different ways to create Office 365 groups. Teams, Planner, SharePoint team sites, etc. — you can accumulate a lot of them very fast. Use this script below to delete the ones you no longer need.
...
foreach ($group in $groups)
{
    ...
    o365 aad o365group remove --id $group.id --confirm
}
```
...
done < <(o365 aad o365group list -o json | jq -r '.[] | .displayName + ";" + .id')
...
for o365group in "${groupstoremove[@]}"; do
  ...
  o365 aad o365group remove --id "$id" --confirm
done
```
