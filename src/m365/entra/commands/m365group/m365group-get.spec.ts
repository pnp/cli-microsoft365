...
describe(commands.M365GROUP_GET, () => {
  ...
  it("shows error when the group is not a unified group", async () => {
    ...
    await assert.rejects(command.action(logger, { options: commandOptionsSchema.parse({ id: groupId }) }), new CommandError(`Specified group with id '${groupId}' is not a Microsoft 365 group.`));
  });
});
