...
class FileListCommand extends SpoCommand {
    ...
    auth
      ...
      .then((res: ContextInfo): request.RequestPromise => {
        ...
        if (args.options.output === 'json') {
          requestUrl = `${args.options.webUrl}/_api/web/GetFolderByServerRelativeUrl('${encodeURIComponent(args.options.folder)}')/Files`;
        }
        else {
          requestUrl = `${args.options.webUrl}/_api/web/GetFolderByServerRelativeUrl('${encodeURIComponent(args.options.folder)}')/Files?$select=UniqueId,Name,ServerRelativeUrl`;
        }
      ...
      .then((fileProperties: FilePropertiesCollection): void => {
        ...
        if (args.options.output === 'json') {
          ...
          cmd.log(fileProperties.value.map(f => {
            return {
              UniqueId: f.UniqueId,
              Name: f.Name,
  ...
  Examples:
    ...
    Return all files from folder ${chalk.grey('shared documents')} located in site ${chalk.grey('https://contoso.sharepoint.com/sites/project-x')}
      ${chalk.grey(config.delimiter)} ${commands.FILE_LIST} --webUrl https://contoso.sharepoint.com/sites/project-x --folder 'share documents'
      `);
  }
}
