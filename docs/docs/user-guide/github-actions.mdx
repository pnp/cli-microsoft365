...
* Using Microsoft Entra ID application id / certificate (perfect in a production context, without creating any dependencies to an account) ==> **application secrets**
...
The `CLI for Microsoft 365 Login` allows you to indicate the targeting tenant (default `common`). It can be `organization` (if using an Microsoft Entra ID app which is multitenant) or a tenant id.
...
| `APP_ID`                | App ID of the Microsoft Entra ID application to use for certificate authentication  | Optional (but required if `authType` is `certificate`) |
