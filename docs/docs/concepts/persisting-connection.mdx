...
To check if you are logged in to Microsoft 365 in the CLI for Microsoft 365, run the `status` command. If you are logged in, the command will return the name of the user account or Microsoft Entra ID used to log in. If you are not connected, the command will return `false`.
