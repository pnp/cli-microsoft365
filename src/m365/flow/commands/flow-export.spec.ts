...
describe(commands.FLOW_EXPORT, () => {
  ...
  it('call is made with suggestedCreationType properties when format specified as ZIP', (done) => {
    ...
    cmdInstance.action({ options: { debug: false, id: `${foundFlowId}`, environment: `Default-${foundEnvironmentId}`, format: 'zip', path: './output.zip' } }, () => {
      try {
        assert.strictEqual(postRequestsStub.lastCall.args[0].body.resources["L1BST1ZJREVSUy9NSUNST1NPRlQuRkxPVy9GTE9XUy9GMkVCOEIzNy1GNjI0LTRCMjItOTk1NC1CNUQwQ0JCMjhGOEI="].suggestedCreationType, 'Update');
        resourceIds.forEach((id) => {
          assert.strictEqual(postRequestsStub.lastCall.args[0].body.resources[id].suggestedCreationType, 'Existing');
        });
        done();
      }
      catch (e) {
