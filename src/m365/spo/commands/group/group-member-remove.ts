...
class SpoGroupMemberRemoveCommand extends SpoCommand {
  ...
  public async commandAction(logger: Logger, args: CommandArgs): Promise<void> {
    if (args.options.force) {
      ...
      const result = await Cli.promptForConfirmation(`Are you sure you want to remove user ${args.options.userName || args.options.userId || args.options.email || args.options.aadGroupId || args.options.aadGroupName} from the SharePoint group?`);
