  ...
  public async commandAction(logger: Logger, args: CommandArgs): Promise<void> {
    try {
      if (!args.options.force) {
        const confirmationResult = await cli.promptForConfirmation({ message: `Are you sure you want to remove model?` });
      ...
      let requestUrl = `${siteUrl}/_api/machinelearning/models/getbyuniqueid('${args.options.id}')`;

      if (args.options.title) {
        requestUrl = `${siteUrl}/_api/machinelearning/models/getbytitle('${formatting.encodeQueryParameter(args.options.title!)}')`;
      }
