...
class SpoSiteDesignGetCommand extends SpoCommand {
  ...
  private getSiteDesignId(args: CommandArgs): Promise<string> {
    ...
    return request
      ...
      .then(response => {
        const siteDesignItem: SiteDesign | undefined = response.value.filter(x => x.Title == args.options.title)[0];

        if (!siteDesignItem) {
          return Promise.reject(`The specified site design does not exist`);
        }

        if (response.value.filter(x => x.Title == args.options.title).length > 1) {
          return Promise.reject(`Multiple site designs with title ${args.options.title} found: ${response.value.filter(x => x.Title == args.options.title).map(x => x.Id)}`);
        }

        return Promise.resolve(siteDesignItem.Id);
      });
  }
