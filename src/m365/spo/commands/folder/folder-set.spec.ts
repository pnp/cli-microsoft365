...
describe(commands.FOLDER_SET, () => {
  ...
  it('fails validation if color is passed as number and color is not an integer', async () => {
    const actual = await command.validate({ options: { webUrl: webUrl, url: folderRelSiteUrl, color: 3.5 } }, commandInfo);
    assert.notStrictEqual(actual, true);
  });

  it('fails validation if color is passed as number and color is smaller than 0', async () => {
    const actual = await command.validate({ options: { webUrl: webUrl, url: folderRelSiteUrl, color: -3 } }, commandInfo);
    assert.notStrictEqual(actual, true);
  });

  it('fails validation if color is passed as number and color is larger than 15', async () => {
    const actual = await command.validate({ options: { webUrl: webUrl, url: folderRelSiteUrl, color: 16 } }, commandInfo);
    assert.notStrictEqual(actual, true);
  });
