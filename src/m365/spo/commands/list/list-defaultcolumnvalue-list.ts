...
class SpoListDefaultColumnValueListCommand extends SpoCommand {
  public get name(): string {
    return commands.LIST_DEFAULTCOLUMNVALUE_LIST;
  }

  public get description(): string {
    return 'Retrieves default column values set for a specific document library';
  }
  ...
  public async commandAction(logger: Logger, args: CommandArgs): Promise<void> {
    try {
      ...
      if (args.options.folderUrl) {
        ...
        defaultValues = defaultValues.filter(d => d.Path.toLowerCase() === serverRelFolderUrl.toLowerCase());
      }
  ...
  private async getServerRelativeListUrl(options: Options): Promise<string> {
    ...
    requestOptions.url += '/RootFolder?$select=ServerRelativeUrl';
...
export default new SpoListDefaultColumnValueListCommand();