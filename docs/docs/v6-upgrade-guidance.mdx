...
We've renamed the `spo group user <verb>` commands to `spo group member <verb>` to better cover all possible scenarios. In the near future we'll be adding support to add Entra ID Groups as group member. Using `spo group member` better fits the intended situation of adding either users or Entra ID groups.
...
[entra sp get](./cmd/entra/sp/sp-get.mdx)|`displayName`|`appDisplayName`
[entra sp get](./cmd/entra/sp/sp-get.mdx)|`objectId`|`appObjectId`
