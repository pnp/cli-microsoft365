...
<Tabs>
  ...
  function Replace-Membership {
    ...
    $groupsToProcess.id | ForEach-Object {
      ...
      $group = $null
      try {
        $group = m365 aad m365group get --id $groupId | Get-CLIValue 
      }
      catch {
        Write-Host
      ...
      $users = $null
      $users = m365 aad m365group user list --groupId $groupId | Get-CLIValue
      $users | Where-Object { $_.userPrincipalName -eq $oldUser } | ForEach-Object {
        ...
        if ($isMember -or $isTeam) {
          try {
            ...
            m365 aad m365group user add --groupId $groupId --userName $newUser | Get-CLIValue
          }
          catch {
        ...
        if ($isOwner) {
          try {
            ...
            m365 aad m365group user add --groupId $groupId --userName $newUser --role Owner | Get-CLIValue
          }
          catch {
            Write-Host $_.Exception.Message -ForegroundColor White
          }
        ...
        try {
          ...
          m365 aad m365group user remove --groupId $groupId --userName $oldUser | Get-CLIValue
        }
        catch {
          Write-Host $_.Exception.Message -ForegroundColor Red
          continue
