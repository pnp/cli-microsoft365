...
describe(commands.LISTITEM_RECORD_UNDECLARE, () => {
  ...
  it('fails validation if the url option is not a valid SharePoint site URL', async () => {
    const actual = await command.validate({ options: { webUrl: 'foo', listItemId: 1 } }, commandInfo);
    assert.notStrictEqual(actual, true);
  });
  ...
  it('fails validation if id is not a number', async () => {
    const actual = await command.validate({ options: { webUrl: 'https://contoso.sharepoint.com', listItemId: 'abc' } }, commandInfo);
    assert.notStrictEqual(actual, true);
  });
});
