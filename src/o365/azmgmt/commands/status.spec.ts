...
describe(commands.STATUS, () => {
  ...
  it('shows disconnected status when not connected (verbose)', (done) => {
    ...
    cmdInstance.action({ options: { verbose: true } }, () => {
      try {
        assert(cmdInstanceLogSpy.calledWith('Not connected to the Azure Management Service'));
        done();
      }
      catch (e) {
        done(e);
    ...
    cmdInstance.action({ options: {} }, () => {
      try {
        assert(cmdInstanceLogSpy.calledWith({ 
          connectedTo: 'https://management.azure.com/',
  ...
  it('correctly reports Azure Management Service resource', (done) => {
    auth.service = new Service('https://management.azure.com/');
    auth.service.connected = true;
    ...
    cmdInstance.action({ options: { debug: true } }, () => {
      ...
      log.forEach(l => {
        if (l && l.aadResource === 'https://management.azure.com/') {
