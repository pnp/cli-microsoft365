...
Viva Engage commands require the delegated 'user_impersonation' permission to be granted for the Microsoft Entra ID application. Viva Engage commands are executed in the context of the currently logged in user. Certificate-based authentication with app_only permissions is currently not supported by Microsoft Entra ID.
