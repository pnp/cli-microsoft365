...
describe(commands.FOLDER_ADD, () => {
  ...
  it('aborts when not connected to a SharePoint site', (done) => {
    ...
    cmdInstance.action = command.action();
    cmdInstance.action({ options: { debug: false, webUrl: 'https://contoso.sharepoint.com', sourceUrl: '/Shared Documents' } }, () => {
      try {
  ...
  it('should correctly pass params to request', (done) => {
    ...
    cmdInstance.action({
      ...
      try {
        ...
        body: { ServerRelativeUrl: '%2FShared%20Documents%2Fabc' },
        json: true }));
        done();
      }
      catch (e) {
      ...
      try {
        ...
        headers:
         { authorization: 'Bearer ABC',
           accept: 'application/json;odata=nometadata',
           'User-Agent': 'NONISV|SharePointPnP|Office365CLI/1.3.0' },
        body: { ServerRelativeUrl: '%2Fsites%2Ftest1%2FShared%20Documents%2Fabc' },
        json: true }));
        done();
      }
      catch (e) {
  ...
  it('fails validation if the webUrl option not specified', () => {
    const actual = (command.validate() as CommandValidate)({ options: { sourceUrl: '/Shared Documents', name: 'My Folder' } });
    assert.notEqual(actual, true);
  });

  it('fails validation if the webUrl option is not a valid SharePoint site URL', () => {
    const actual = (command.validate() as CommandValidate)({ options: { webUrl: 'foo', sourceUrl: '/Shared Documents', name: 'My Folder' } });
    assert.notEqual(actual, true);
  });

  it('passes validation if the webUrl option is a valid SharePoint site URL and sourceUrl specified', () => {
    const actual = (command.validate() as CommandValidate)({ options: { webUrl: 'https://contoso.sharepoint.com', sourceUrl: '/Shared Documents', name: 'My Folder' } });
  ...
  it('fails validation if the name option not specified', () => {
    const actual = (command.validate() as CommandValidate)({ options: { webUrl: 'https://contoso.sharepoint.com', sourceUrl: '/Shared Documents' } });
    assert.notEqual(actual, true);
  });
