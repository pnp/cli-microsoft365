...
Before you can use CLI for Microsoft 365 commands to manage your tenant, you have to log in to Microsoft 365. This step assumes you already set up the CLI and have a Microsoft Entra app registration ready.
...
If you don't have an application registration yet, start with the [Setup the CLI](setup-cli.mdx) guide. It shows the quickest path with `m365 setup` and points you to [Use your own Microsoft Entra identity](../using-own-identity.mdx) when you need to register an app manually.
...
For more details on creating an application registration or setting the environment variables, refer to [Use your own Microsoft Entra identity](../using-own-identity.mdx).
