...
class SpoTermGroupAddCommand extends SpoCommand {
  ...
  Remarks:
    ...
    To add a taxonomy term group, you have to first log in to a tenant admin
  ...
  Examples:
    ...
    Add a new taxonomy term group with the specified name and description
      ${chalk.grey(config.delimiter)} ${commands.TERM_GROUP_GET} --name PnPTermSets --description 'Term sets for PnP'
`);
  }
}
