...
describe(commands.HIDEDEFAULTTHEMES_GET, () => {
  ...
  it('logs correct telemetry event', (done) => {
    ...
    cmdInstance.action({ options: {} }, () => {
      try {
        assert.equal(telemetry.name, commands.HIDEDEFAULTTHEMES_GET);
        done();
      }
      catch (e) {
        done(e);
  ...
  it('gets the current value of the HideDefaultThemes setting for JSON output', (done) => {
    let expected: any = {"value": true};
    sinon.stub(request, 'post').callsFake((opts) => {
      if (opts.url.indexOf('/_api/thememanager/GetHideDefaultThemes') > -1) {
        if (opts.headers.authorization &&
          opts.headers.authorization.indexOf('Bearer ') === 0 &&
