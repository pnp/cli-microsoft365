...
describe(commands.LIST_VIEW_FIELD_ADD, () => {
  ...
  it('add the field by title to viewId and listUrl', async () => {
    ...
    await command.action(logger, { options: { debug: false, webUrl: 'https://contoso.sharepoint.com/sites/ninja', listUrl: '/sites/ninja/Shared Documents', viewId: 'cc27a922-8224-4296-90a5-ebbc54da2e81', fieldTitle: 'Created By' } });
    let correctRequestIssued = false;
    requests.forEach(r => {
  ...
  it('move the field by title to the position index to viewId of listTitle (debug)', async () => {
    ...
    await command.action(logger, { options: { debug: true, verbose: true, webUrl: 'https://contoso.sharepoint.com/sites/ninja', listTitle: 'Documents', viewId: 'cc27a922-8224-4296-90a5-ebbc54da2e81', fieldTitle: 'Created By', fieldPosition: 1 } });
    let correctRequestIssued = false;
    requests.forEach(r => {
  ...
  it('move the field by title to the position index to viewTitle of listId', async () => {
    ...
    await command.action(logger, { options: { debug: false, verbose: true, webUrl: 'https://contoso.sharepoint.com/sites/ninja', listId: '0cd891ef-afce-4e55-b836-fce03286cccf', viewTitle: 'MyView', fieldTitle: 'Created By', fieldPosition: 1 } });
    let correctRequestIssued = false;
    requests.forEach(r => {
      if (r.url.indexOf(`/viewfields/moveviewfieldto`) > -1 &&
        r.headers.accept &&
