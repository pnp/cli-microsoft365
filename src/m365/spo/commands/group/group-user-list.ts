...
class SpoGroupUserListCommand extends SpoCommand {
  ...
  public commandAction(logger: Logger, args: CommandArgs, cb: (err?: any) => void): void {
    if (this.verbose) {
      logger.logToStderr(`Retrieving the list of members from the SharePoint group :  ${args.options.groupId ? args.options.groupId : args.options.name}`);
    }
    ...
    const requestUrl: string = `${args.options.webUrl}/_api/web/sitegroups/${args.options.groupId
      ...
      : `GetByName('${encodeURIComponent(args.options.name as string)}')`}/users`;
    ...
    request
      .get<GroupUserInfo[]>(requestOptions)
      .then((groupUserList: GroupUserInfo[]): void => {
        logger.log(groupUserList);
        cb();
  ...
  public validate(args: CommandArgs): boolean | string {
    if (args.options.groupId && args.options.name) {
      return 'Use either "name" or "groupId", but not both';
    }

    if (!args.options.groupId && !args.options.name) {
      return 'Either "name" or "groupId" is required';
    }
