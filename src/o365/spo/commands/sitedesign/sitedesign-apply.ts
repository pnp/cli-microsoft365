...
class SpoSiteDesignApplyCommand extends SpoCommand {
  ...
  public get description(): string {
    return 'Applies a published site design to a specified site collection';
  }

  public getTelemetryProperties(args: CommandArgs): any {
    const telemetryProps: any = super.getTelemetryProperties(args);
    telemetryProps.webUrl = args.options.webUrl;
  ...
  public validate(): CommandValidate {
    return (args: CommandArgs): boolean | string => {
      ...
      else if (!Utils.isValidGuid(args.options.id)) {
  ...
  Remarks:
    ...
    To apply a site design to an existing site collection, you have to first connect to a SharePoint site
    using the ${chalk.blue(commands.CONNECT)} command,
    eg. ${chalk.grey(`${config.delimiter} ${commands.CONNECT} https://contoso.sharepoint.com`)}.

  Examples:

    Apply the site design with ID ${chalk.grey('9b142c22-037f-4a7f-9017-e9d8c0e34b98')} to the site collection ${chalk.grey('https://contoso.sharepoint.com/sites/project-x')}
