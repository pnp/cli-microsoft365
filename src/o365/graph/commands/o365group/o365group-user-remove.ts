...
class GraphO365GroupUserRemoveCommand extends GraphCommand {
  ...
  public commandAction(cmd: CommandInstance, args: CommandArgs, cb: () => void): void {
    ...
    const removeUser: () => void = (): void => {
      auth
        ...
        .then((res: { value: string; }): request.RequestPromise => {
          ...
          providedUserId = res.value;

          const requestOptions: any = {
            url: `${auth.service.resource}/v1.0/groups/${providedGroupId}/owners?$select=id,displayName,userPrincipalName,userType`,
            headers: Utils.getRequestHeaders({
              authorization: `Bearer ${auth.service.accessToken}`,
              accept: 'application/json;odata.metadata=none'
            }),
        ...
        .then((res: any): Promise<void> | request.RequestPromise => {
          ...
          const endpoint: string = `${auth.service.resource}/v1.0/groups/${providedGroupId}/${userIsOwner ? 'owners' : 'members'}/${providedUserId}/$ref`;
    ...
    if (args.options.confirm) {
      ...
      cmd.prompt({
        ...
        name: 'continue',
        default: false,
        message: `Are you sure you want to remove ${args.options.userName} from the group or team ${providedGroupId}?`,
  ...
  Remarks:
    ...
    To remove a user from the specified Office 365 Group or Microsoft Teams team, you have to first
    log in to the Microsoft Graph using the ${chalk.blue(commands.LOGIN)} command,
    eg. ${chalk.grey(`${config.delimiter} ${commands.LOGIN}`)}.

    You can remove users from a Office 365 Group or Microsoft Teams team if you are owner of that
    group or team.
