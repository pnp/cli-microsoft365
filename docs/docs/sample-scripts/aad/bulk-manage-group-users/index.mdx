...
<Tabs>
  ...
  ForEach ($taskItem in $taskItems) {
      ...
      If ($action -eq "add") {
          ...
          If ($role -eq "owner") {
              m365 aad m365group user add --groupId $group.id --userName $user.mail --role Owner
              Write-Host $user.mail " added as owner in " $group.mailNickname
          }
          ElseIf ($role -eq "member") {
              m365 aad m365group user add --groupId $group.id --userName $user.mail
      ...
      ElseIf ($action -eq "remove") {
          m365 aad m365group user remove --groupId $group.id --userName $user.mail --force
