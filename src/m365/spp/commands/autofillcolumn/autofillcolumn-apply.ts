...
class SppAutofillcolumnApplyCommand extends SpoCommand {
  public get name(): string {
    return commands.AUTOFILLCOLUMN_APPLY;
  }
    ...
    try {
      ...
      const requestOptions: CliRequestOptions = {
        ...
        data: {
          ...
          syntexPoweredColumnPrompts: JSON.stringify([{ columnId: column.Id, columnName: column.Title, prompt: args.options.prompt, isEnabled: true }])
        }
      };
  ...
  private getColumn(options: Options, listId: string): Promise<Field> {
    ...
    const requestOptions: CliRequestOptions = {
      url: `${options.siteUrl}/_api/web/lists(guid'${formatting.encodeQueryParameter(listId)}')/fields${fieldRestUrl}`,
      headers: {
        accept: 'application/json;odata=nometadata'
      },
      responseType: 'json'
...
export default new SppAutofillcolumnApplyCommand();