  ...
  it('calls api with correct params using federated identity flow from GitHub Action', async () => {
    ...
    const requestGetStub = sinon.stub(request, 'get').callsFake(async (opts) => {
      if (opts.url === `${process.env.ACTIONS_ID_TOKEN_REQUEST_URL}&audience=${encodeURIComponent(federatedIdentityAudience)}` &&
        opts.headers?.Authorization === `Bearer ${process.env.ACTIONS_ID_TOKEN_REQUEST_TOKEN}`
      ) {
        return {
    ...
    const requestPostStub = sinon.stub(request, 'post').callsFake(async (opts) => {
      if (opts.url === `https://login.microsoftonline.com/${tenant}/oauth2/v2.0/token` &&
        opts.data.indexOf(`client_id=${appId}`) > -1 &&
        opts.data.indexOf(`client_assertion=${federatedIdentityClientAssertion}`) > -1
      ) {
        return {
    ...
    assert.strictEqual(requestGetStub.firstCall.args[0].url, `${process.env.ACTIONS_ID_TOKEN_REQUEST_URL}&audience=${encodeURIComponent(federatedIdentityAudience)}`);
    assert.strictEqual(requestPostStub.firstCall.args[0].url, `https://login.microsoftonline.com/${tenant}/oauth2/v2.0/token`);
  });
