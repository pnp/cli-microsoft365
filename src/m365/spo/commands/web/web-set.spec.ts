...
describe(commands.WEB_SET, () => {
  ...
  it('Update Welcome page', async () => {
    sinon.stub(request, 'patch').callsFake((opts) => {
      if (opts.url === 'https://contoso.sharepoint.com/sites/team-a/_api/web/RootFolder' || opts.url === 'https://contoso.sharepoint.com/sites/team-a/_api/web') {
        return Promise.resolve();
      }
  ...
  it('correctly handles error while updating Welcome page', async () => {
    sinon.stub(request, 'patch').callsFake((opts) => {
      if (opts.url === 'https://contoso.sharepoint.com/sites/team-a/_api/web/RootFolder') {
        return Promise.reject('The WelcomePage property must be a path that is relative to the folder, and the path cannot contain two consecutive periods (..).');
      }
