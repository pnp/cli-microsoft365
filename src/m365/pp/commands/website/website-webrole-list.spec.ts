...
describe(commands.WEBSITE_WEBROLE_LIST, () => {
  ...
  it('retrieves webroles for Power Pages site by id', async () => {
    ...
    await command.action(logger, { options: commandOptionsSchema.parse({ verbose: true, environmentName: environment, websiteId: '4916bb2c-91e1-4716-91d5-b6171928fac9' }) });
  ...
  it('retrieves webroles for Power Pages site by name', async () => {
    ...
    await command.action(logger, { options: commandOptionsSchema.parse({ verbose: true, environmentName: environment, websiteName: 'Site 1' }) });
    assert(loggerLogSpy.calledWith(webroleResponse));
  });
  ...
  it('correctly handles API OData error', async () => {
    ...
    await assert.rejects(command.action(logger, { options: commandOptionsSchema.parse({ environmentName: environment, websiteId: '4916bb2c-91e1-4716-91d5-b6171928fac9' }) }),
