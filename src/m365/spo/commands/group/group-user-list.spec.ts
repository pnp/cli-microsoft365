...
describe(commands.GROUP_USER_LIST, () => {
  ...
  it('fails validation if webURL is Invalid', () => {
    const actual = command.validate({ options: { webUrl: "InvalidWEBURL", groupId: 3 } });
    assert.notStrictEqual(actual, true);
  });

  it('fails validation if groupid and groupname is entered', () => {
    const actual = command.validate({ options: { webUrl: "https://contoso.sharepoint.com/sites/SiteA", groupId: "4", name: "Contoso Site Owners" } });
    assert.notStrictEqual(actual, true);
  });

  it('fails validation if neither groupid nor groupname is entered', () => {
    const actual = command.validate({ options: { webUrl: "https://contoso.sharepoint.com/sites/SiteA" } });
    assert.notStrictEqual(actual, true);
  });

  it('fails validation if groupID is Invalid', () => {
    const actual = command.validate({ options: { webUrl: "https://contoso.sharepoint.com/sites/SiteA", groupId: "INVALIDGROUP" } });
    assert.notStrictEqual(actual, true);
  });

  it('passes validation if all the required options are specified', () => {
    const actual = command.validate({ options: { webUrl: "https://contoso.sharepoint.com/sites/SiteA", groupId: 3 } });
