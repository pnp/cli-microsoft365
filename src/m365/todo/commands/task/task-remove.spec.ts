...
describe(commands.TASK_REMOVE, () => {
  ...
  it('removes a To Do task by task id and task list name (verbose)', (done) => {
  ...
  it('fails validation when id parameter is not valid with listId', () => {
    const actual = command.validate({
      options: {
        debug: false,
        id: null,
  ...
  it('fails validation when id parameter is not valid with listName', () => {
    const actual = command.validate({
      options: {
        debug: false,
        id: null,
