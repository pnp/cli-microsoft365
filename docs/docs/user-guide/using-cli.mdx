...
Remember, instead of escaping, it's also possible to [feed complex content from a file](./using-cli.mdx#passing-complex-content-into-cli-options). 
...
CLI for Microsoft 365 by default outputs errors to stderr. Unfrortunately, PowerShell doesn't handle stderr as well as other shells. Also in case of command errors PowerShell does not break execution and may still continue even after setting `$ErrorActionPreference = "Stop"`. In order to overcome those problems you may use the following settings and helper method.

```PowerShell
m365 cli config set --key output --value json # setting output to json
m365 cli config set --key errorOutput --value stdout # changing error output to stdout
m365 cli config set --key showHelpOnFailure --value false # don't show help on command failure
m365 cli config set --key printErrorsAsPlainText --value false # don't print errors as plain text but as objects
...
With the above settings and helper method, you can now use the CLI for Microsoft 365 in PowerShell and have it behave as expected. Example usage:

```PowerShell
try {
  # some script here like 
  m365 spo list list --webUrl "random" | Invoke-CLICommand
}
catch {
  Write-Host "Errors: $($_.Exception.Message) "
}
```
