...
class AadAppRoleAssignmentAddCommand extends AadCommand {
      ...
      .then((res: { value: ServicePrincipal[] }): Promise<AppRole[]> => {
        ...
        for (const scope of args.options.scope.split(',')) {
          ...
          if (!existingRoles.length) {
            ...
            appRolesFound.map((r: AppRole) => availableRoles += `\n${r.value}`);

            return Promise.reject(`The scope value '${scope}' you have specified does not exist for ${args.options.resource}. \nAvailable scopes (application permissions) are: ${availableRoles}`);
          }
  ...
  public options(): CommandOption[] {
    const options: CommandOption[] = [
      {
        option: '--appId [appId]',
        description: 'Application appId also known as clientId of the App Registration on which the configured scopes (app roles) should be applied'
      },
      {
        option: '--objectId [objectId]',
        description: 'Application objectId of the App Registration on which the configured scopes (app roles) should be applied'
      },
      {
        option: '--displayName [displayName]',
        description: 'Application name of the App Registration on which the configured scopes (app roles) should be applied'
      },
      {
  ...
  public commandHelp(args: {}, log: (help: string) => void): void {
    ...
    The autocomplete will just suggest some known names, but that would not restrict the users 
    to use name of their own custom application or other application within their tenant.

    This command can also be used to assign permissions to system or user-assigned managed identity.
   
  Examples:
  
    Adds SharePoint Sites.Read.All application permissions to Azure AD application with 
    app id ${chalk.grey('57907bf8-73fa-43a6-89a5-1f603e29e451')}
      ${commands.APPROLEASSIGNMENT_ADD} --appId "57907bf8-73fa-43a6-89a5-1f603e29e451" --resource "SharePoint" --scope "Sites.Read.All"

    Adds multiple Microsoft Graph application permissions to an Azure AD application with
    name ${chalk.grey('MyAppName')}
      ${commands.APPROLEASSIGNMENT_ADD} --displayName "MyAppName" --resource "Microsoft Graph" --scope "Mail.Read,Mail.Send"

    Adds Microsoft Graph Mail.Read application permissions to 
    a system managed identity app with objectId ${chalk.grey('57907bf8-73fa-43a6-89a5-1f603e29e451')}
