...
describe('Auth', () => {
  ...
  it('uses browser auth and retrieves an unsuccessful response', (done) => {
    ...
    initializeServerStub.restore();
    initializeServerStub = sinon.stub((auth as any)._authServer, 'initializeServer').callsFake((service: Service, resource: string, resolve: (error: InteractiveAuthorizationCodeResponse) => void, reject: (error: ErrorResponse) => void) => { 
      reject(error); 
    });
    sinon.stub(auth as any, 'storeConnectionInfo').callsFake(() => Promise.resolve());
