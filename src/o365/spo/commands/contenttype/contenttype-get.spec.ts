...
describe(commands.CONTENTTYPE_GET, () => {
  ...
  it('logs correct telemetry event', (done) => {
    ...
    cmdInstance.action({ options: {} }, () => {
      try {
        assert.equal(telemetry.name, commands.CONTENTTYPE_GET);
        done();
      }
      catch (e) {
        done(e);
  ...
  it('correctly handles lack of valid access token', (done) => {
    ...
    auth.site.url = 'https://contoso.sharepoint.com';
    cmdInstance.action = command.action();
    cmdInstance.action({ options: { debug: true, webUrl: 'https://contoso.sharepoint.com/sites/sales', id: '0x0100558D85B7216F6A489A499DB361E1AE2F' } }, (err?: any) => {
      try {
