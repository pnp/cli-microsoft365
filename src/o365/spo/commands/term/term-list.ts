...
class SpoTermListCommand extends SpoCommand {
  ...
  Examples:
    ...
    List taxonomy terms from the term group and term set with the given name
      ${chalk.grey(config.delimiter)} ${commands.TERM_SET_LIST} --termGroupName PnPTermSets --termSetName PnP-Organizations

    List taxonomy terms from the term group and term set with the given ID
      ${chalk.grey(config.delimiter)} ${commands.TERM_SET_LIST} --termGroupId 0e8f395e-ff58-4d45-9ff7-e331ab728beb --termSetId 0e8f395e-ff58-4d45-9ff7-e331ab728bec
`);
  }
}
