...
If the existing members or owners are not specified in the `memberIds`, `memberUserNames`, `ownerIds`, or `ownerUserNames` options, they will be removed from the group.
