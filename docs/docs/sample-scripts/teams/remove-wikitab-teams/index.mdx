...
<Tabs>
  ...
  $groups = m365 aad m365group list --query "[?mailNickname=='$groupMailNickname']" -o json | ConvertFrom-Json
  ...
  groups=$(m365 aad m365group list -o json | jq '.[] | select(.mailNickname == "'"$groupMailNickname"'")')
  if [ -z "$groups" ]; then
