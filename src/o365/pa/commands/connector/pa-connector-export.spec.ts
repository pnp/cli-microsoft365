...
const command: Command = require('./pa-connector-export');
...
describe(commands.CONNECTOR_EXPORT, () => {
  ...
  it('exports the custom connectors', (done) => {
    ...
    sinon.stub(request, 'get').callsFake((opts) => {
      if (opts.url.indexOf(`providers/Microsoft.PowerApps/apis/shared_connector-201-5f20a1f2d8d6777a75-5fa602f410652f4dfa?api-version=2016-11-01&$filter=environment%20eq%20%27Default-5be1aa17-e6cd-4d3d-8355-01af3e607d4b%27%20and%20IsCustomApi%20eq%20%27True%27`) > -1) {
        ...
        if (opts.headers['x-anonymous'] === true) {
  ...
  it('passes validation when the specified output folder exists', () => {
    sinon.stub(fs, 'existsSync').callsFake((folder) => folder.indexOf('connector') < 0);
    const actual = (command.validate() as CommandValidate)({ options: { environment: 'Default-d87a7535-dd31-4437-bfe1-95340acd55c5', connector: 'shared_connector-201-5f20a1f2d8d6777a75-5fa602f410652f4dfa', outputFolder: '123' } });
    assert.equal(actual, true);
  });
