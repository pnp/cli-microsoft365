...
describe(commands.LIST_SITESCRIPT_GET, () => {
  ...
  it('logs correct telemetry event', (done) => {
    ...
    cmdInstance.action({ options: {} }, () => {
      try {
        assert.equal(telemetry.name, commands.LIST_SITESCRIPT_GET);
        done();
      }
      catch (e) {
        done(e);
  ...
  it('correctly handles error when trying to extract site script if the server did not return generated site script (using listId)', (done) => {
    ...
    cmdInstance.action({
      ...
      try {
        assert.equal(JSON.stringify(err), JSON.stringify(new CommandError("An error occured, the site script could not be extracted from list 'fb4b0cf8-c006-4802-a1ea-57e0e4852188'")));
        done();
      }
      catch (e) {
        done(e);
  ...
  it('correctly handles error when trying to extract site script if the server did not return generated site script (using listTitle)', (done) => {
    ...
    cmdInstance.action({
      ...
      try {
        assert.equal(JSON.stringify(err), JSON.stringify(new CommandError("An error occured, the site script could not be extracted from list 'MyLibrary'")));
        done();
      }
      catch (e) {
        done(e);
