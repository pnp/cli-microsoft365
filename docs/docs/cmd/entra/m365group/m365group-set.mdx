...
: Indicates whether the signed-in user is subscribed to receive email conversations. Valid values: `true`, `false`.
```
...
The options `allowExternalSenders` and `autoSubscribeNewMembers` can only be set using delegated permissions.
...
Set-up the Microsoft 365 group so that the signed-in user is subscribed to receive email conversations.

```sh
m365 entra m365group set --id 28beab62-7540-4db1-a23f-29a6018a3848 --isSubscribedByMail
```
