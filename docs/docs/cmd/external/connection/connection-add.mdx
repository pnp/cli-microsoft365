...
```md definition-list
`-i, --id <id>`
: Developer-provided unique ID of the connection within the Microsoft Entra ID tenant.
...
: Comma-separated collection of application IDs for registered Microsoft Entra ID apps that are allowed to manage the external connection and to index content in the external connection.
```
