...
describe(commands.APPROLEASSIGNMENT_REMOVE, () => {
  ...
  it('deletes app role assignments for service principal with specified displayName', async () => {
    await command.action(logger, { options: { appDisplayName: 'myapp', resource: 'SharePoint', scope: 'Sites.Read.All', force: true } });
    assert(deleteRequestStub.called);
  });

  it('deletes app role assignments for service principal with specified objectId and multiple scopes', async () => {
    await command.action(logger, { options: { appObjectId: '3e64c22f-3f14-4bce-a267-cb44c9a08e17', resource: 'SharePoint', scope: 'Sites.Read.All,Sites.FullControl.All', force: true } });
  ...
  it('deletes app role assignments for service principal with specified appId (debug)', async () => {
    await command.action(logger, { options: { debug: true, appId: 'dc311e81-e099-4c64-bd66-c7183465f3f2', resource: 'SharePoint', scope: 'Sites.Read.All', force: true } });
    assert(deleteRequestStub.called);
  });
  ...
  it('handles exchange alias for the resource option value', async () => {
    await command.action(logger, { options: { debug: true, appId: 'dc311e81-e099-4c64-bd66-c7183465f3f2', resource: 'exchange', scope: 'Sites.Read.All', force: true } });
    assert(deleteRequestStub.called);
  });

  it('handles appId for the resource option value', async () => {
    await command.action(logger, { options: { debug: true, appId: 'dc311e81-e099-4c64-bd66-c7183465f3f2', resource: 'fff194f1-7dce-4428-8301-1badb5518201', scope: 'Sites.Read.All', force: true } });
    assert(deleteRequestStub.called);
  });
  ...
  it('rejects if no resource is found', async () => {
    ...
    await assert.rejects(command.action(logger, { options: { debug: true, appId: '3e64c22f-3f14-4bce-a267-cb44c9a08e17', resource: 'SharePoint', scope: 'Sites.Read.All', force: true } } as any),
  ...
  it('rejects if app role assignment is not found', async () => {
    await assert.rejects(command.action(logger, { options: { debug: true, appId: '3e64c22f-3f14-4bce-a267-cb44c9a08e17', resource: 'SharePoint', scope: 'Sites.ReadWrite.All', force: true } } as any),
