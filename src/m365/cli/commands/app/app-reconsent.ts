...
import { Application, RequiredResourceAccess, ServicePrincipal } from '@microsoft/microsoft-graph-types';
...
class CliAppReconsentCommand extends GraphCommand {
  ...
  public async commandAction(logger: Logger): Promise<void> {
    try {
      ...
      const application = await this.getApplicationByAppId(appId);
  ...
  private async getApplicationByAppId(appId: string): Promise<Application> {
    const apps = await odata.getAllItems<Application>(`${this.resource}/v1.0/applications?$filter=appId eq '${appId}'&$select=requiredResourceAccess,id`);
    return apps[0];
  }
  ...
  private async addCliAppScopes(logger: Logger, appScopes: RequiredResourceAccess[]): Promise<void> {
    ...
    const servicePrincipals = await odata.getAllItems<ServicePrincipal>(`${this.resource}/v1.0/servicePrincipals?$select=displayName,appId,oauth2PermissionScopes,servicePrincipalNames`);
