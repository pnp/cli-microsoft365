...
class SpoTenantCdnPolicyListCommand extends Command {
  ...
  public get action(): CommandAction {
    return function (args: CommandArgs, cb: () => void) {
      ...
      appInsights.trackEvent({
        name: commands.TENANT_CDN_POLICY_LIST,
        properties: {
      ...
      auth
        ...
        .then((res: ContextInfo): Promise<string> => {
          ...
          this.log(`Retrieving configured policies for ${(cdnType === 1 ? 'Private' : 'Public')} CDN...`);
