...
describe(commands.SITE_ADMIN_LIST, () => {
  ...
  it('gets site collection admins in regular mode', async () => {
    sinon.stub(request, 'get').callsFake(async opts => {
      if (opts.url === `${siteUrl}/_api/web/siteusers?$filter=IsSiteAdmin%20eq%20true`) {
  ...
  it('correctly handles error when site is not found for specified site URL admin mode', async () => {
    ...
    await assert.rejects(command.action(logger, { options: { siteUrl: siteUrl, asAdmin: true } }), new CommandError(`Failed to obtain site Id from the provided site URL.`));
  });
  ...
  it('correctly handles error when site id is not found for specified site URL in admin mode', async () => {
    ...
    await assert.rejects(command.action(logger, { options: { siteUrl: siteUrl, asAdmin: true } }), new CommandError('Unknown Error'));
  });
