...
.EXAMPLE
    PS C:\> Send-SiteCollectionStorageReport -storageThreshold 70 -sendTo john.smith@contoso.com
    Running this function with send an email to John Smith with all the sites over 70% storage used.
.INPUTS
    Inputs (if any)
.OUTPUTS
    Output (if any)
