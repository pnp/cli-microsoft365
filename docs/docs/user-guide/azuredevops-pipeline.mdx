...
Next in the top right corner of the page click on the `New pipline` button. After that, follow the for by selecting the location of your repo, next the branch on which the YAML file is present, and then select the option to create a new pipeline based on `Existing Azure Pipelines YAML file`
