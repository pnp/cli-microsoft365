...
const command: Command = require('./teams-set');
import * as assert from 'assert';
import request from '../../../../request';
import Utils from '../../../../Utils';
import { Service } from '../../../../Auth';

describe(commands.TEAMS_SET, () => {
  let vorpal: Vorpal;
  let log: string[];
  let cmdInstance: any;
  ...
  it('has correct name', () => {
    assert.equal(command.name.startsWith(commands.TEAMS_SET), true);
  });
  ...
  it('logs correct telemetry event', (done) => {
    ...
    cmdInstance.action({ options: {} }, () => {
      try {
        assert.equal(telemetry.name, commands.TEAMS_SET);
        done();
      }
      catch (e) {
        done(e);
  ...
  it('has help referring to the right command', () => {
    ...
    cmd.help({}, () => { });
    assert(find.calledWith(commands.TEAMS_SET));
  });
