...
Creates new Entra ID app registration
...
`--manifest [manifest]`
: Entra ID app manifest as retrieved from the Azure Portal to create the app registration from. Specify either `name` or `manifest` but not both.
...
You can use this command to create a new Entra registration either by specifying the different configuration settings through the corresponding options or by using the manifest.
...
Create new EntraD app registration with Application ID URI set to a value that contains the ID of the app registration, designated with the `_appId_` token and a custom API scope that can be consented by both admins and users
