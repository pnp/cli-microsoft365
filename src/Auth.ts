...
export class Auth {
  ...
  public restoreAuth(): Promise<void> {
    return new Promise<void>(
      (resolve: () => void, reject: (error: any) => void): void => {
        this.getServiceConnectionInfo<Service>().then(
  ...
  public ensureAccessToken(
    ...
    return new Promise<string>(
      (
        ...
        const accessToken: AccessToken | undefined = this.service.accessTokens[
          resource
        ];
        ...
        let getTokenPromise: (
          ...
          logger: Logger,
          debug: boolean
        ) => Promise<TokenResponse> = this.ensureAccessTokenWithDeviceCode.bind(
          this
        );
        ...
        if (this.service.refreshToken) {
          ...
          switch (this.service.authType) {
            ...
            case AuthType.Certificate:
              getTokenPromise = this.ensureAccessTokenWithCertificate.bind(
                this
              );
              break;
        ...
        getTokenPromise(resource, logger, debug)
          .then(
            (tokenResponse: TokenResponse): Promise<void> => {
  ...
  private ensureAccessTokenWithPassword(
    resource: string,
    logger: Logger,
    debug: boolean
  ): Promise<TokenResponse> {
  ...
  private ensureAccessTokenWithCertificate(
    resource: string,
    logger: Logger,
    debug: boolean
  ): Promise<TokenResponse> {
    return new Promise<TokenResponse>(
      (
        ...
        if (debug) {
          logger.logToStderr(
            `Retrieving new access token using certificate (thumbprint ${this.service.thumbprint})...`
          );
        }
  ...
  private ensureAccessTokenWithWindowsCertificate(
    resource: string,
    logger: Logger,
    debug: boolean
  ): Promise<TokenResponse> {
  ...
  private ensureAccessTokenWithIdentity(
    resource: string,
    logger: Logger,
    debug: boolean
  ): Promise<TokenResponse> {
    return new Promise<TokenResponse>(
      (
        ...
        if (debug) {
          logger.logToStderr(
            "Wil try to retrieve access token using identity..."
          );
        }
        ...
        if (process.env.IDENTITY_ENDPOINT && process.env.IDENTITY_HEADER) {
          ...
          requestOptions.headers["X-IDENTITY-HEADER"] =
        ...
        request
          ...
          .then((res: any): void => {
            resolve({
              ...
              expiresOn: parseInt(res.expires_on) * 1000,
            } as any);
            return;
          })
          .catch((e: any) => {
            ...
            if (e.error && e.error.Message) {
              ...
              isNotFoundResponse =
                e.error.error_description === "Identity not found";
            }
  ...
  private getServiceConnectionInfo<TConn>(): Promise<TConn> {
    return new Promise<TConn>(
      (
        resolve: (connectionInfo: TConn) => void,
        reject: (error: any) => void
      ): void => {
        const tokenStorage = this.getTokenStorage();
        tokenStorage.get().then(
          (json: string): void => {
            try {
