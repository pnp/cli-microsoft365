...
class SpoTermListCommand extends SpoCommand {
  ...
  public async commandAction(logger: Logger, args: CommandArgs): Promise<void> {
    ...
    try {
      ...
      const shouldIncludeChildTerms: boolean | undefined = args.options.includeChildTerms || args.options.withChildTerms;
  ...
  private async getChildTerms(spoWebUrl: string, res: ContextInfo, parentTerm: Term): Promise<void> {
    parentTerm.Children = [];
  ...
  private async executeCsomCall(data: string, spoWebUrl: string, res: ContextInfo): Promise<TermCollection> {
