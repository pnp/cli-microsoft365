...
describe(commands.KNOWLEDGEHUB_SET, () => {
  ...
  it('sets the Knowledgehub Site', (done) => {
    command.action(logger, { options: { debug: false, siteUrl: 'https://contoso.sharepoint.com/sites/knowledgesite' } }, () => {
  ...
  it('sets the Knowledgehub Site (debug)', (done) => {
    command.action(logger, { options: { debug: true, siteUrl: 'https://contoso.sharepoint.com/sites/knowledgesite' } }, () => {
      let setRequestIssued = false;
      requests.forEach(r => {
        if (r.url.indexOf('/_vti_bin/client.svc/ProcessQuery') > -1 &&
          r.headers['X-RequestDigest'] &&
  ...
  it('correctly handles an error when setting Knowledgehub Site', (done) => {
    ...
    command.action(logger, { options: { debug: false, siteUrl: 'https://contoso.sharepoint.com/sites/knowledgesite' } } as any, (err?: any) => {
      try {
