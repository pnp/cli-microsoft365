...
describe(commands.CARD_GET, () => {
  ...
  it('fails validation when no card found', async () => {
  ...
  it('retrieves a specific card with the name parameter', async () => {
    ...
    await command.action(logger, { options: { debug: true, environment: '4be50206-9576-4237-8b17-38d8aadfaa36', name: 'CLI 365 Card' } });
    assert(loggerLogSpy.calledWith(cardResponse.value[0]));

  });

  it('retrieves a specific card with the name parameter as admin', async () => {
    sinon.stub(powerPlatform, 'getDynamicsInstanceApiUrl').returns(envUrl);
    ...
    await command.action(logger, { options: { debug: true, environment: '4be50206-9576-4237-8b17-38d8aadfaa36', name: 'CLI 365 Card', asAdmin: true } });
    assert(loggerLogSpy.calledWith(cardResponse.value[0]));
  ...
  it('retrieves a specific card with the id parameter as admin', async () => {
    sinon.stub(powerPlatform, 'getDynamicsInstanceApiUrl').returns(envUrl);
  ...
  it('correctly handles API OData error', async () => {
    ...
    await assert.rejects(command.action(logger, { options: { debug: false, environment: '4be50206-9576-4237-8b17-38d8aadfaa36', name: 'CLI 365 Card' } } as any),
