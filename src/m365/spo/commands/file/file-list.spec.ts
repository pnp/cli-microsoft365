...
describe(commands.FILE_LIST, () => {
  ...
  it('retrieves files from a folder when --recursive option is not supplied', async () => {
    sinon.stub(request, 'get').callsFake(async (opts) => {
      if (opts.url === `https://contoso.sharepoint.com/sites/project-x/_api/web/GetFolderByServerRelativeUrl(@url)/Files?@url='${formatting.encodeQueryParameter('/sites/project-x/' + folderUrl)}'&$skip=0&$top=5000`) {
  ...
  it('retrieves files from a folder with filter and fields option, requesting the ListItemAllFields Id property', async () => {
    sinon.stub(request, 'get').callsFake(async (opts) => {
      if (opts.url === `https://contoso.sharepoint.com/sites/project-x/_api/web/GetFolderByServerRelativeUrl(@url)/Files?@url='${formatting.encodeQueryParameter('/sites/project-x/' + folderUrl)}'&$skip=0&$top=5000&$expand=ListItemAllFields&$select=ListItemAllFields/Id,Name&$filter=name eq 'Test.docx'`) {
        return {
          value: [
            {
              ListItemAllFields: {
  ...
  it('retrieves files from a folder with filter and fields option, requesting the ListItemAllFields Title property', async () => {
    sinon.stub(request, 'get').callsFake(async (opts) => {
      if (opts.url === `https://contoso.sharepoint.com/sites/project-x/_api/web/GetFolderByServerRelativeUrl(@url)/Files?@url='${formatting.encodeQueryParameter('/sites/project-x/' + folderUrl)}'&$skip=0&$top=5000&$expand=ListItemAllFields&$select=ListItemAllFields/Title&$filter=name eq 'Test.docx'`) {
        return {
          value: [
            {
              ListItemAllFields: {
  ...
  it('retrieves files from a folder in multiple pages', async () => {
    sinon.stub(request, 'get').callsFake(async (opts) => {
      ...
      if (opts.url === `https://contoso.sharepoint.com/sites/project-x/_api/web/GetFolderByServerRelativeUrl(@url)/Files?@url='${formatting.encodeQueryParameter('/sites/project-x/' + folderUrl)}'&$skip=5000&$top=5000`) {
        return fileShortArrayResponse;
      }
  ...
  it('retrieves files from a folder recursively in multiple pages', async () => {
    sinon.stub(request, 'get').callsFake(async (opts) => {
      ...
      if (opts.url === `https://contoso.sharepoint.com/sites/project-x/_api/web/GetFolderByServerRelativeUrl(@url)/Folders?@url='${formatting.encodeQueryParameter('/sites/project-x/' + folderUrl)}'&$skip=0&$top=5000&$select=ServerRelativeUrl`) {
        return folderFullPageResponse;
      }

      if (opts.url === `https://contoso.sharepoint.com/sites/project-x/_api/web/GetFolderByServerRelativeUrl(@url)/Folders?@url='${formatting.encodeQueryParameter('/sites/project-x/' + folderUrl)}'&$skip=5000&$top=5000&$select=ServerRelativeUrl`) {
  ...
  it('retrieves files from a folder when --recursive option is not supplied and output option is text', async () => {
    sinon.stub(request, 'get').callsFake(async (opts) => {
      if (opts.url === `https://contoso.sharepoint.com/sites/project-x/_api/web/GetFolderByServerRelativeUrl(@url)/Files?@url='${formatting.encodeQueryParameter('/sites/project-x/' + folderUrl)}'&$skip=0&$top=5000&$select=UniqueId,Name,ServerRelativeUrl`) {
