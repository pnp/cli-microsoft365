...
```sh
m365 entra pim roleassignment add [options]
```
...
: Id of the directory object representing the scope of the assignment. If not specified, will default to tenant-wide scope.
...
To use this command you must be **Privileged Role Administrator**.

:::

When specifying the scope of the assignment, use 

- `'/'` for tenant-wide scope
- `'/administrativeUnits/{administrative_unit_id}'` for scope limited to the administrative unit
- `'/{application_object_id}'` for scope limited to the application
...
```sh
m365 entra pim roleassignment add --roleDefinitionName 'SharePoint Administrator'
```

Request activation of an Entra ID role assignment for the current user.

```sh
m365 entra pim roleassignment add --roleDefinitionId 'f1417aa3-bf0b-4cc5-a845-a0b2cf11f690'
```

Request activation of an Entra ID role assignment for the current user with a justification and max duration of 4 hours.

```sh
m365 entra pim roleassignment add --roleDefinitionId 'f1417aa3-bf0b-4cc5-a845-a0b2cf11f690' --justification 'Need Global Admin to release application xyz to production' --duration 'PT4H'
```

Request activation of an Entra ID role assignment for a specified user with tenant scope.

```sh
m365 entra pim roleassignment add --roleDefinitionId 'f1417aa3-bf0b-4cc5-a845-a0b2cf11f690' --userId '3488d6b8-6b2e-41c3-9583-1991205323c2' --directoryScopeId '/'
```

Request activation of the User Administrator Entra ID role assignment for a specified group with administrative unit scope.

```sh
m365 entra pim roleassignment add --roleDefinitionName 'User Administrator' --groupId '3488d6b8-6b2e-41c3-9583-1991205323c2' --directoryScopeId '/administrativeUnits/03c4c9dc-6f0c-4c4f-a4e6-0c9ed80f54c7'
```

Request activation of the Application Administrator Entra ID role assignment for a specified group with scope to a specific application.

```sh
m365 entra pim roleassignment add --roleDefinitionName 'Application Administrator' --groupName 'Applications admins' --directoryScopeId '/03c4c9dc-6f0c-4c4f-a4e6-0c9ed80f54c7'
```

Request activation of an Entra ID role assignment for a specific period of two days.

```sh
m365 entra pim roleassignment add --roleDefinitionName 'Global Administrator' --userName 'admin-john@contoso.com' --startDateTime '2024-01-10T09:00:00Z' --endDateTime '2024-01-11T17:00:00Z'
```
...
- Role assignment request: https://learn.microsoft.com/graph/api/rbacapplication-post-roleassignmentschedulerequests?view=graph-rest-1.0