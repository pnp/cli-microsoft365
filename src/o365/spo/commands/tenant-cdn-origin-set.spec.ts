...
const tenantCdnOriginSetCommand: Command = require('./tenant-cdn-origin-set');
import * as assert from 'assert';
import * as request from 'request-promise-native';
import config from '../../../config';
import Utils from '../../../Utils';

describe(commands.TENANT_CDN_ORIGIN_SET, () => {
  let vorpal: Vorpal;
  let log: string[];
  let cmdInstance: any;
  let trackEvent: any;
  ...
  it('has correct name', () => {
    assert.equal(tenantCdnOriginSetCommand.name.startsWith(commands.TENANT_CDN_ORIGIN_SET), true);
  });

  it('has a description', () => {
    assert.notEqual(tenantCdnOriginSetCommand.description, null);
  });

  it('calls telemetry', (done) => {
    cmdInstance.action = tenantCdnOriginSetCommand.action();
  ...
  it('logs correct telemetry event', (done) => {
    ...
    cmdInstance.action({ options: {}, appCatalogUrl: 'https://contoso.sharepoint.com/sites/appcatalog' }, () => {
      try {
        assert.equal(telemetry.name, commands.TENANT_CDN_ORIGIN_SET);
        done();
      }
      catch (e) {
        done(e);
  ...
  it('sets CDN origin on the public CDN when Public type specified', (done) => {
    ...
    cmdInstance.action = tenantCdnOriginSetCommand.action();
    cmdInstance.action({ options: { verbose: true, origin: '*/cdn', type: 'Public' } }, () => {
  ...
  it('sets CDN origin on the private CDN when Private type specified', (done) => {
    ...
    auth.site.tenantId = 'abc';
    cmdInstance.action = tenantCdnOriginSetCommand.action();
    cmdInstance.action({ options: { verbose: true, origin: '*/cdn', type: 'Private' } }, () => {
      let setRequestIssued = false;
      requests.forEach(r => {
        if (r.url.indexOf('/_vti_bin/client.svc/ProcessQuery') > -1 &&
          r.headers.authorization &&
  ...
  it('sets CDN origin on the public CDN when no type specified', (done) => {
    ...
    auth.site.tenantId = 'abc';
    cmdInstance.action = tenantCdnOriginSetCommand.action();
    cmdInstance.action({ options: { verbose: false, origin: '*/cdn' } }, () => {
      let setRequestIssued = false;
      requests.forEach(r => {
        if (r.url.indexOf('/_vti_bin/client.svc/ProcessQuery') > -1 &&
          r.headers.authorization &&
  ...
  it('supports verbose mode', () => {
    const options = (tenantCdnOriginSetCommand.options() as CommandOption[]);
  ...
  it('accepts Public SharePoint Online CDN type', () => {
    const actual = (tenantCdnOriginSetCommand.validate() as CommandValidate)({ options: { type: 'Public' } });
    assert(actual);
  });

  it('accepts Private SharePoint Online CDN type', () => {
    const actual = (tenantCdnOriginSetCommand.validate() as CommandValidate)({ options: { type: 'Private' } });
    assert(actual);
  });

  it('rejects invalid SharePoint Online CDN type', () => {
    const type = 'foo';
    const actual = (tenantCdnOriginSetCommand.validate() as CommandValidate)({ options: { type: type } });
  ...
  it('doesn\'t fail validation if the optional type option not specified', () => {
    const actual = (tenantCdnOriginSetCommand.validate() as CommandValidate)({ options: {} });
    assert(actual);
  });
  ...
  it('has help referring to the right command', () => {
    ...
    (tenantCdnOriginSetCommand.help() as CommandHelp)({}, helpLog);
    assert(find.calledWith(commands.TENANT_CDN_ORIGIN_SET));
  });
  ...
  it('has help with examples', () => {
    ...
    (tenantCdnOriginSetCommand.help() as CommandHelp)({}, log);
