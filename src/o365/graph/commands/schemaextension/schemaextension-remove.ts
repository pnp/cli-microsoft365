...
class GraphSchemaExtensionRemove extends GraphCommand {
  public get name(): string {
    return `${commands.SCHEMAEXTENSION_REMOVE}`;
  }

  public get description(): string {
    return 'Removes specified MS Graph schema extension';
  }
  ...
  public commandAction(cmd: CommandInstance, args: CommandArgs, cb: () => void): void {
    const removeSchemaExtension: () => void = (): void => {
        if (this.verbose) {
          cmd.log(`Removes specified MS Graph schema extension with id '${args.options.id}'...`);
        }
...
module.exports = new GraphSchemaExtensionRemove();