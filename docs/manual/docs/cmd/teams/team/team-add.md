...
`-d, --description [description]`|Description for the Microsoft Teams team. Required if `templatePath` supplied
`--templatePath [templatePath]`|Local path to the file containing the template. If `name` or `description` are supplied, these take prescendence over the template values
