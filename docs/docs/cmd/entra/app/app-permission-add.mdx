...
Adds the specified application and/or delegated permissions to a specified Entra (Microsoft Entra ID) app
...
```md definition-list
`-i, --appId [appId]`
: Client ID of the Microsoft Entra ID app to add the API permissions to. Specify `appId` or `appObjectId` but not both.

`--appObjectId [appObjectId]`
: Object ID of the Microsoft Entra ID app to add the API permissions to. Specify `appId` or `appObjectId` but not both.
...
Grant multiple delegated API permissions to an Microsoft Entra ID app specified by client id
...
Grant multiple delegated API permissions to an Microsoft Entra ID app specified by object id
...
Grant multiple app-only permissions to an Microsoft Entra ID app specified by client id and grant admin consent
