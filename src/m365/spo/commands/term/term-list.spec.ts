...
describe(commands.TERM_LIST, () => {
  ...
  it('gets taxonomy terms including child terms from term set by id, term group by id using includeChildTerms parameter', async () => {
  ...
  it('gets taxonomy terms including child terms from term set by id, term group by id in a friendly formatted way using includeChildTerms parameter', async () => {
    const termGroupId = '0e8f395e-ff58-4d45-9ff7-e331ab728beb';
    const termSetId = '7a167c47-2b37-41d0-94d0-e962c1a4f2ed';
    sinon.stub(request, 'post').callsFake(async (opts) => {
      if (opts.url === 'https://contoso-admin.sharepoint.com/_vti_bin/client.svc/ProcessQuery' && opts.headers && opts.headers['X-RequestDigest']) {
