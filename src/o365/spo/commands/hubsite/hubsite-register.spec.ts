...
describe(commands.HUBSITE_REGISTER, () => {
  ...
  it('registers site as a hub site', (done) => {
    ...
    cmdInstance.action = command.action();
    cmdInstance.action({ options: { debug: false, siteCollectionUrl: 'https://contoso.sharepoint.com/sites/sales' } }, () => {
      try {
  ...
  it('registers site as a hub site (debug)', (done) => {
    ...
    auth.site.url = 'https://contoso.sharepoint.com';
    cmdInstance.action = command.action();
    cmdInstance.action({ options: { debug: true, siteCollectionUrl: 'https://contoso.sharepoint.com/sites/sales' } }, () => {
      try {
        assert(cmdInstanceLogSpy.calledWith({
          "Description": null,
          "ID": "255a50b2-527f-4413-8485-57f4c17a24d1",
  ...
  it('correctly handles error when trying to register site which already is a hub site as a hub site', (done) => {
    ...
    cmdInstance.action({ options: { debug: false, siteCollectionUrl: 'https://contoso.sharepoint.com/sites/sales' } }, () => {
      try {
        assert(cmdInstanceLogSpy.calledWith(new CommandError('This site is already a HubSite.')));
        done();
      }
      catch (e) {
        done(e);
  ...
  it('supports specifying site collection URL', () => {
    ...
    let containsOption = false;
    options.forEach(o => {
      if (o.option.indexOf('--siteCollectionUrl') > -1) {
        containsOption = true;
      }
    });
    assert(containsOption);
  ...
  it('fails validation if the specified site collection URL is not a valid SharePoint URL', () => {
    const actual = (command.validate() as CommandValidate)({ options: { siteCollectionUrl: 'site.com' } });
    assert.notEqual(actual, true);
  });

  it('passes validation when all required parameters are valid', () => {
    const actual = (command.validate() as CommandValidate)({ options: { siteCollectionUrl: 'https://contoso.sharepoint.com/sites/sales' } });
