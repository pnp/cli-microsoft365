...
class EntraAppRoleRemoveCommand extends GraphCommand {
  ...
  private async getAppObjectId(args: CommandArgs, logger: Logger): Promise<string> {
    ...
    const result: { id: string } = (await cli.handleMultipleResultsFound(`Multiple Microsoft Entra application registration with name '${appName}' found.`, resultAsKeyValuePair)) as { id: string };
    return result.id;
  }
}
