...
class O365GroupReportActivityCountsCommand extends GraphCommand {
  ...
  public commandAction(cmd: CommandInstance, args: CommandArgs, cb: () => void): void {
    const endpoint: string = `${this.resource}/v1.0/reports/getOffice365GroupsActivityCounts(period='${encodeURIComponent(args.options.period)}')`;
  ...
  public options(): CommandOption[] {
    const options: CommandOption[] = [
      ...
      },
      {
        ...
        description: 'Path to the file where the Office 365 Groups activities across group workloads report should be stored in'
      }
    ];
  ...
  public commandHelp(args: {}, log: (help: string) => void): void {
    ...
    and exports the report data in the specified path in text format
      ${commands.O365GROUP_REPORT_ACTIVITYCOUNTS} --period D7 --output text --outputFile './o365groupactivitycounts.txt'

    Get the number of group activities across group workloads for the last week
    and exports the report data in the specified path in json format
      ${commands.O365GROUP_REPORT_ACTIVITYCOUNTS} --period D7 --output json --outputFile './o365groupactivitycounts.json'
`);
  }
}
