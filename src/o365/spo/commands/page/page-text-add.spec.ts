...
describe(commands.PAGE_TEXT_ADD, () => {
	...
	it('logs correct telemetry event', (done) => {
		...
		cmdInstance.action({ options: {} }, () => {
			try {
				assert.equal(telemetry.name, commands.PAGE_TEXT_ADD);
				done();
			} catch (e) {
				done(e);
			}
	...
	it('adds a text to a modern page', (done) => {
	...
	it('adds a text to a modern page without specifying the page extension', (done) => {
		fakeClientSidePageRestAPISuccess();
		auth.site = new Site();
		auth.site.connected = true;
		auth.site.url = 'https://contoso.sharepoint.com';
			...
			() => {
				try {
					const hasVerboseInfo =
						log.length > 0 &&
						log.filter((l) => l == 'Retrieving request digest...').length > 0;
					assert(hasVerboseInfo);
					done();
				} catch (e) {
					done(e);
				}
	...
	it('correctly handles OData error when adding text to modern page', (done) => {
	...
	it('correctly handles invalid value for column when adding text to modern page', (done) => {
		fakeClientSidePageRestAPISuccess();
		auth.site = new Site();
		auth.site.connected = true;
		auth.site.url = 'https://contoso.sharepoint.com';
	...
	it('correctly handles invalid value for section when adding text to modern page', (done) => {
		fakeClientSidePageRestAPISuccess();
		auth.site = new Site();
		auth.site.connected = true;
		auth.site.url = 'https://contoso.sharepoint.com';
	...
	it('correctly handles null value for column when adding text to modern page', (done) => {
		fakeClientSidePageRestAPISuccess();
		auth.site = new Site();
		auth.site.connected = true;
		auth.site.url = 'https://contoso.sharepoint.com';
	...
	it('supports specifying text', () => {
		const options = command.options() as CommandOption[];
		let containsOption = false;
		options.forEach((o) => {
			if (o.option.indexOf('--text') > -1) {
				containsOption = true;
			}
		});
		assert(containsOption);
  ...
  it('fails validation if text parameter is not specified', () => {
    const actual = (command.validate() as CommandValidate)({
  ...
  it('passes validation when name, webUrl and text are specified', () => {
    const actual = (command.validate() as CommandValidate)({
      options: { pageName: 'page.aspx', webUrl: 'https://contoso.sharepoint.com', text: 'Hello World!' }
    });
    assert.equal(actual, true);
  });
