...
class PaSolutionInitCommand extends Command {
  ...
  public commandHelp(args: CommandArgs, log: (help: string) => void): void {
    ...
    PublisherName only allows characters within the ranges [A - Z], [a - z], [0 - 9], or _.
    The first character may only be in the ranges [A - Z], [a - z], or _.
    
    PublisherPrefix must be 2 to 8 characters long, can only consist of alpha-numerics,
    must start with a letter, and cannot start with 'mscrm'.

  Examples:

    Initializes a CDS solution project using ${chalk.grey('yourPublisherName')} as publisher name
    and ${chalk.grey('ypn')} as publisher prefix
