...
describe(commands.AUTOFILLCOLUMN_SET, () => {
  ...
  it('correctly handles error when applying autofill without the required prompt parameter.', async () => {
    ...
    await assert.rejects(command.action(logger, { options: { siteUrl: 'https://contoso.sharepoint.com/sites/sales', columnId: '9b1b1e42-794b-4c71-93ac-5ed92488b67f', listId: '421b1e42-794b-4c71-93ac-5ed92488b67d' } }), new CommandError('The prompt parameter is required when setting the autofill column for the first time.'));
  });
});