...
`-s, --secret [secret]`
: Client Secret of the Microsoft Entra ID application to use for authentication. Required when `authType` is set to `secret`.
...
When connecting to clouds other than `Public`, you'll need to use an Microsoft Entra ID application registered in a directory provisioned in that cloud. If you try to login using the default Microsoft Entra ID application, login will fail.
