...
describe(commands.HUBSITE_LIST, () => {
  ...
  it('correctly retrieves the associated sites in batches using includeAssociatedSites parameter', async () => {
  ...
  it('correctly retrieves the associated sites in batches using includeAssociatedSites parameter (debug)', async () => {
    // Cast the command class instance to any so we can set the private
    // property 'batchSize' to a small number for easier testing
    const newBatchSize = 3;
    (command as any).batchSize = newBatchSize;
  ...
  it('lists hub sites, including associated sites, with all properties for JSON output using includeAssociatedSites parameter', async () => {
    sinon.stub(request, 'get').resolves({
      value: [
        {
          "Description": null,
  ...
  it('correctly handles empty result while retrieving associated sites in batches using includeAssociatedSites parameter', async () => {
    // Cast the command class instance to any so we can set the private
    // property 'batchSize' to a small number for easier testing
    const newBatchSize = 3;
    (command as any).batchSize = newBatchSize;
  ...
  it('correctly handles error in the first batch of associated sites using includeAssociatedSites parameter', async () => {
    const error = {
      error: {
        'odata.error': {
  ...
  it('correctly handles error in a subsequent batch of associated sites using includeAssociatedSites parameter', async () => {
    const error = {
      error: {
        'odata.error': {
          code: '-1, Microsoft.SharePoint.Client.InvalidOperationException',
