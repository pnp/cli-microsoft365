...
class SpoFileCopyCommand extends SpoCommand {
  ...
  public commandAction(cmd: CommandInstance, args: CommandArgs, cb: () => void): void {
    ...
    auth
      ...
      .then((accessToken: string): request.RequestPromise => {
         ...
         * Called by purpose, we explicitly check if user specified file
   ...
   * @param sourceUrl server relative url
   * @param cmd cmd
   */
  public fileExists(tenantUrl: string, webUrl: string, sourceUrl: string, siteAccessToken: string, cmd: any): request.RequestPromise {
   ...
   * @param targetUrl Server-relative URL where to copy the file server relative url
   * @param filename file name of the file to be copied
   * @param cmd cmd
   */
  public recycleFile(tenantUrl: string, targetUrl: string, filename: string, siteAccessToken: string, cmd: any): Promise<void> {
  ...
  public options(): CommandOption[] {
    const options: CommandOption[] = [
      ...
      },
      {
        ...
        description: 'If a file already exists at the targetUrl, it will be moved to the recycle bin. If ommitted, the copy operation will be canceled if the file already exists at the targetUrl location'
      }
    ];
  ...
  Remarks:
    ...
    To copy a file, you have to first connect to SharePoint using the
  ...
  Examples:
    ...
    Performs file copy between two site collections for file with name ${chalk.grey('sp1.pdf')}
    ...
    Performs file copy between two document libraries in the same site collection for file with name ${chalk.grey('sp1.pdf')}
    located in site document library ${chalk.grey('https://contoso.sharepoint.com/sites/test1/Shared%20Documents')}
    ...
    Performs file copy between two site collections for file with name ${chalk.grey('sp1.pdf')}
    located in site document library ${chalk.grey('https://contoso.sharepoint.com/sites/test1/Shared%20Documents')}with option --deleteIfAlreadyExists. This will delete existing file with the same name in the target folder.
