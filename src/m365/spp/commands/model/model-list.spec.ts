...
describe(commands.MODEL_LIST, () => {
  ...
  it('correctly handles site is not Content Site', async () => {
    ...
    await assert.rejects(command.action(logger, { options: { debug: true, siteUrl: 'https://contoso.sharepoint.com/sites/portal' } } as any),
  ...
  it('retrieves all site models', async () => {
    ...
    sinon.stub(request, 'get').callsFake(async (opts) => {
      ...
      if ((opts.url as string).indexOf(`/_api/machinelearning/models`) > -1) {
        return { value: models };
      }
    ...
    assert(loggerLogSpy.calledWith(models));
  });
