...
describe('FN003003_CFG_bundles', () => {
  ...
  it('returns notification if bundles is not up-to-date', () => {
    const project: any = {
      path: '/usr/tmp',
      configJson: {
      }
  ...
  it('should correctly convert entries schema to bundle schema', () => {
    ...
    assert.notEqual(bundle1, undefined);
    assert.equal(bundle1.components[0].entrypoint, './lib/extensions/helloWorld/HelloWorldApplicationCustomizer.js');
    assert.equal(bundle1.components[0].manifest, './src/extensions/helloWorld/HelloWorldApplicationCustomizer.manifest.json');
    assert.equal(bundle1.components[0].outputPath, undefined);
  });
