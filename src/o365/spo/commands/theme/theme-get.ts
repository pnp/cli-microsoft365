...
class ThemeGetCommand extends SpoCommand {
  ...
  public get description(): string {
    return 'Get custom theme details from tenant for the given theme name';
  }
  ...
  public options(): CommandOption[] {
    const options: CommandOption[] = [{
        option: '-n, --name <name>',
        description: 'name of the theme to retreive'
      }
    ];
  ...
  public commandHelp(args: {}, log: (help: string) => void): void {
    ...
    Remarks:
      ...
      To get theme, you have to first connect to SharePoint using the
      ${chalk.blue(commands.CONNECT)} command, eg. ${chalk.grey(`${config.delimiter} ${commands.CONNECT} https://contoso-admin.sharepoint.com`)}.
          
    Examples:
    
      To get theme from the tenant
      ${chalk.grey(config.delimiter)} ${commands.THEME_GET} -n Contoso-Blue
      `);
  }
}
