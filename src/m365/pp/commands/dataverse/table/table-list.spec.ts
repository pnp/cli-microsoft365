...
describe(commands.DATAVERSE_TABLE_LIST, () => {
  ...
  it('correctly handles API OData error', async () => {
    ...
    try {
      await command.action(logger, { options: { debug: false, environment: 'doesnotexist' } });
    }
    catch (ex) {
      assert(ex, (new CommandError("Error: The environment 'doesnotexist' could not be found")).message);
    }
