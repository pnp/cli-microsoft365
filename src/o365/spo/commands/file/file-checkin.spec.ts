...
describe(commands.FILE_CHECKIN, () => {
  ...
  it('passes validation url, type and comment params correct', () => {
    const actual = (command.validate() as CommandValidate)({ options: { webUrl: 'https://contoso.sharepoint.com', url: '/sites/docs/abc.txt', type: 'overwrite', comment: '123' } });
    assert.equal(actual, true);
  });

  it('passes validation type is major', () => {
    const actual = (command.validate() as CommandValidate)({ options: { webUrl: 'https://contoso.sharepoint.com', url: '/sites/docs/abc.txt', type: 'major' } });
    assert.equal(actual, true);
  });

  it('passes validation type is minor', () => {
    const actual = (command.validate() as CommandValidate)({ options: { webUrl: 'https://contoso.sharepoint.com', url: '/sites/docs/abc.txt', type: 'minor' } });
    assert.equal(actual, true);
  });
