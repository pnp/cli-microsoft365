...
class GraphGroupSettingSetCommand extends GraphCommand {
  ...
  Remarks:
    ...
    To update a group setting, you have to first connect to
    ...
    You can retrieve the ID of the group setting using the ${chalk.blue(commands.GROUPSETTING_LIST)}
    command.

    To update values for the different properties specified in the group
    setting, include additional options that match the property in the
    group setting.
    ...
    If you don't specify a value for the particular property, it will remain unchanged.
    To find out which properties are available for the particular group setting, use the
    ${chalk.blue(commands.GROUPSETTING_GET)} command.

    If the specified ${chalk.blue('id')} doesn't reference a valid group setting,
    you will get a ${chalk.grey("Resource 'xyz' does not exist or one of its ")}
