...
class SiteAppCatalogAddCommand extends SpoCommand {
  ...
  public get description(): string {
    return 'Adds site collection scoped app catalog to site';
  }
  ...
  public options(): CommandOption[] {
    const options: CommandOption[] = [
      {
        option: '-u, --url [url]',
        description: 'URL of the site collection where the app catalog to add'
      }
    ];
  ...
  Remarks:
    ...
    To add site collection app catalogs, you have to first connect to a tenant admin site using the
  ...
  Examples:
    ...
    Add a site collection app catalog to the specified site
      ${chalk.grey(config.delimiter)} ${commands.SITE_APPCATALOG_ADD} --url https://contoso.sharepoint.com/sites/site007
    `);
  }
}
