...
class OutlookReportMailAppUsageAppsUserCountsCommand extends PeriodBasedReport {
  public get name(): string {
    return commands.OUTLOOK_REPORT_MAILAPPSUSAGEAPPSUSERCOUNTS;
  }
  ...
  public commandHelp(args: {}, log: (help: string) => void): void {
    ...
    Gets the count of unique users per email app for the last week
      ${commands.OUTLOOK_REPORT_MAILAPPSUSAGEAPPSUSERCOUNTS} --period D7

    Gets the count of unique users per email app for the last week
    and exports the report data in the specified path in text format
      ${commands.OUTLOOK_REPORT_MAILAPPSUSAGEAPPSUSERCOUNTS} --period D7 --output text --outputFile 'C:/report.txt'

    Gets  details about mailbox usage for the last week
    and exports the report data in the specified path in json format
      ${commands.OUTLOOK_REPORT_MAILAPPSUSAGEAPPSUSERCOUNTS} --period D7 --output json --outputFile 'C:/report.json'
`);
  }
}
