...
describe(commands.YAMMER_SEARCH, () => {
  ...
  it('returns the summary', function (done) {
    ...
    command.action(logger, { options: { search:"contents" } } as any, (err?: any) => {
      try {
        assert.strictEqual(loggerSpy.lastCall.args[0].messages, 4)
        assert.strictEqual(loggerSpy.lastCall.args[0].groups, 2)
        assert.strictEqual(loggerSpy.lastCall.args[0].topics, 5)
        assert.strictEqual(loggerSpy.lastCall.args[0].users, 4)
        done();
      }
      catch (e) {
        done(e);
  ...
  it('returns limited results', function (done) {
    ...
    command.action(logger, { options: { search:"contents", limit: 1, output: "json"  } } as any, (err?: any) => {
      try {
        assert.strictEqual(loggerSpy.lastCall.args[0].summary.messages, 4, "summary returns 4 messages")
        assert.strictEqual(loggerSpy.lastCall.args[0].summary.groups, 2, "sumamry returns 2 groups")
        assert.strictEqual(loggerSpy.lastCall.args[0].summary.topics, 5, "summary return two topics")
        assert.strictEqual(loggerSpy.lastCall.args[0].summary.users, 4, "summary returns 4 users")
        assert.strictEqual(loggerSpy.lastCall.args[0].messages.length, 1, "message arary returns 1 message")
        assert.strictEqual(loggerSpy.lastCall.args[0].groups.length, 1, "groups array returns 1 group")
        assert.strictEqual(loggerSpy.lastCall.args[0].topics.length, 1, "topics array returns 1 topic")
        assert.strictEqual(loggerSpy.lastCall.args[0].users.length, 1, "users array returns 1 user")
        done();
      }
      catch (e) {
        done(e);
  ...
  it('returns all results', (done) => {
    ...
    command.action(logger, { options: { search:"contents", output: "json" } } as any, (err?: any) => {
      try {
        ...
        assert.strictEqual(loggerSpy.lastCall.args[0].summary.topics, 5, "summary return two topics")
        assert.strictEqual(loggerSpy.lastCall.args[0].summary.users, 4, "summary returns 4 users")
        assert.strictEqual(loggerSpy.lastCall.args[0].messages.length, 4, "message arary returns 4 entries")
        assert.strictEqual(loggerSpy.lastCall.args[0].groups.length, 2, "groups array returns 2 groups")
        assert.strictEqual(loggerSpy.lastCall.args[0].topics.length, 5, "topics array returns 2 topics")
        assert.strictEqual(loggerSpy.lastCall.args[0].users.length, 4, "users array returns 4 users")
        done();
      }
      catch (e) {
        done(e);
