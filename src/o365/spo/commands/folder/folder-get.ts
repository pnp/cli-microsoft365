...
class SpoFolderGetCommand extends FolderBaseCommand {
    ...
    auth
      ...
      { 
        if(err.statusCode && err.statusCode === 500) {
          cmd.log('Please check the folder URL. Folder might not exist on the specified URL');
        }
    ...
    const options: CommandOption[] = [
      {
        ...
        description: 'The URL of the site where the folder is'
      },
      {
  ...
  Remarks:
    ...
    To get information about a folder, you have to first connect to SharePoint using the
    ${chalk.blue(commands.CONNECT)} command, eg. ${chalk.grey(`${config.delimiter} ${commands.CONNECT} https://contoso.sharepoint.com`)}.
        
  Examples:
  
    Get folder properties for folder with site relative url ${chalk.grey('/Shared Documents')}
