...
const tenantCdnOriginRemoveCommand: Command = require('./tenant-cdn-origin-remove');
import * as assert from 'assert';
import * as request from 'request-promise-native';
import config from '../../../config';
import Utils from '../../../Utils';

describe(commands.TENANT_CDN_ORIGIN_REMOVE, () => {
  let vorpal: Vorpal;
  let log: string[];
  let cmdInstance: any;
  let trackEvent: any;
  ...
  it('has correct name', () => {
    assert.equal(tenantCdnOriginRemoveCommand.name.startsWith(commands.TENANT_CDN_ORIGIN_REMOVE), true);
  });

  it('has a description', () => {
    assert.notEqual(tenantCdnOriginRemoveCommand.description, null);
  });
  ...
  it('supports verbose mode', () => {
    const options = (tenantCdnOriginRemoveCommand.options() as CommandOption[]);
  ...
  it('accepts Public SharePoint Online CDN type', () => {
    const actual = (tenantCdnOriginRemoveCommand.validate() as CommandValidate)({ options: { type: 'Public' } });
    assert(actual);
  });

  it('accepts Private SharePoint Online CDN type', () => {
    const actual = (tenantCdnOriginRemoveCommand.validate() as CommandValidate)({ options: { type: 'Private' } });
    assert(actual);
  });

  it('rejects invalid SharePoint Online CDN type', () => {
    const type = 'foo';
    const actual = (tenantCdnOriginRemoveCommand.validate() as CommandValidate)({ options: { type: type } });
  ...
  it('doesn\'t fail validation if the optional type option not specified', () => {
    const actual = (tenantCdnOriginRemoveCommand.validate() as CommandValidate)({ options: {} });
    assert(actual);
  });
  ...
  it('has help referring to the right command', () => {
    ...
    (tenantCdnOriginRemoveCommand.help() as CommandHelp)({}, helpLog);
    assert(find.calledWith(commands.TENANT_CDN_ORIGIN_REMOVE));
  });
  ...
  it('has help with examples', () => {
    ...
    (tenantCdnOriginRemoveCommand.help() as CommandHelp)({}, log);
