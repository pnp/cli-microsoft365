...
const command: Command = require('./listitem-retentionlabel-record-lock');

describe(commands.LISTITEM_RETENTIONLABEL_RECORD_LOCK, () => {
  let log: any[];
  let logger: Logger;
  ...
  it('has correct name', () => {
    assert.strictEqual(command.name, commands.LISTITEM_RETENTIONLABEL_RECORD_LOCK);
  });
