...
When specifying a custom Azure AD application to be used by the CLI for Microsoft 365, set the `CLIMICROSOFT365_ENTRAAPPID` environment variable to the ID of your Azure AD application.
...
For instructions on how to create your own Azure AD app see [Using your own Azure AD identity](../user-guide/using-own-identity.mdx).
...
Viva Engage commands require the delegated 'user_impersonation' permission to be granted for the Azure AD application. Viva Engage commands are executed in the context of the currently logged in user. Certificate-based authentication with app_only permissions is currently not supported by Azure AD.
