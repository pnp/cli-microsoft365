...
class SpoPropertyBagGetCommand extends SpoPropertyBagBaseCommand {
  ...
  public validate(): CommandValidate {
    return (args: CommandArgs): boolean | string => {
      if (!args.options.key) {
        return `${args.options.key} is required`;
      }
  ...
  Remarks:
    ...
    To retrieve property bag value, you have to first connect to a SharePoint Online site using the
    ${chalk.blue(commands.CONNECT)} command, eg. ${chalk.grey(`${config.delimiter} ${commands.CONNECT} https://contoso.sharepoint.com`)}.

  Examples:
    Returns propery bag value with key key1 located in site ${chalk.grey('https://contoso.sharepoint.com/sites/test')}
      ${chalk.grey(config.delimiter)} ${commands.PROPERTYBAG_GET} -w https://contoso.sharepoint.com/sites/test -k key1
    
    Returns propery bag value with key key1 located in site root folder ${chalk.grey('https://contoso.sharepoint.com/sites/test')}
      ${chalk.grey(config.delimiter)} ${commands.PROPERTYBAG_GET} -w https://contoso.sharepoint.com/sites/test -k key1 -f /

    Returns propery bag value with key key1 located in site document library ${chalk.grey('https://contoso.sharepoint.com/sites/test')}
    ...
    Returns propery bag value with key key1 located in folder in site document library ${chalk.grey('https://contoso.sharepoint.com/sites/test')}
      ${chalk.grey(config.delimiter)} ${commands.PROPERTYBAG_GET} -w https://contoso.sharepoint.com/sites/test -k key1 -f '/Shared Documents/MyFolder'

    Returns propery bag value with key key1 located in site list ${chalk.grey('https://contoso.sharepoint.com/sites/test')}
  ...
  More information:
    ...
    SharePoint Patterns and Practices (PnP) PowerShell, Get-PnPProperty:
      https://docs.microsoft.com/en-us/powershell/module/sharepoint-pnp/get-pnppropertybag?view=sharepoint-ps
      `);
  }
