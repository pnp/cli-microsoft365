...
class SpoReportActivityUserCountsCommand extends PeriodBasedReport {
  ...
  Examples:
    ...
    and exports the report data in the specified path in text format
      ${commands.REPORT_ACTIVITYUSERCOUNTS} --period D7 --output text --outputFile 'report.txt'

    Gets the trend in the number of active users for the last week
    and exports the report data in the specified path in json format
      ${commands.REPORT_ACTIVITYUSERCOUNTS} --period D7 --output json --outputFile 'report.json'
`);
  }
}
