...
import { FN006002_CFG_PS_skipFeatureDeployment } from './FN006002_CFG_PS_skipFeatureDeployment';

describe('FN006002_CFG_PS_skipFeatureDeployment', () => {
  let findings: Finding[];
  let rule: FN006002_CFG_PS_skipFeatureDeployment;

  beforeEach(() => {
    findings = [];
    rule = new FN006002_CFG_PS_skipFeatureDeployment('boolean');
  });
  ...
  it('returns notification if skipFeatureDeployment value is string while boolean required', () => {
    const project: Project = {
      path: '/usr/tmp',
      packageSolutionJson: {
        $schema: 'test-schema',
