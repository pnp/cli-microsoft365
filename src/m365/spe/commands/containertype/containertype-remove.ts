...
class SpeContainerTypeRemoveCommand extends SpoCommand {
  ...
  private async getContainerTypeId(options: Options, spoAdminUrl: string, logger: Logger): Promise<string> {
    ...
    if (containerTypes.length > 1) {
      ...
      const containerType = await cli.handleMultipleResultsFound<ContainerTypeProperties>(`Multiple container types with name '${options.name}' found.`, containerTypeKeyValuePair);
