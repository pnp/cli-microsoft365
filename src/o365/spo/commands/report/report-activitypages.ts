...
class SpoReportActivityPagesCommand extends PeriodBasedReport {
  ...
  public commandHelp(args: {}, log: (help: string) => void): void {
    ...
    and exports the report data in the specified path in text format
      ${commands.REPORT_ACTIVITYPAGES} --period D7 --output text --outputFile 'C:/report.txt'

    Gets the number of unique pages visited by users for the last week
    and exports the report data in the specified path in json format
      ${commands.REPORT_ACTIVITYPAGES} --period D7 --output json --outputFile 'C:/report.json'
`);
  }
}
