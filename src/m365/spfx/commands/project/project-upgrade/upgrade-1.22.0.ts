...
` }),
  new FN015015_FILE_config_typescript_json({
    add: true, contents: `{
...
` }),
  ...
  new FN021004_PKG_scripts_build({ script: 'heft test --clean --production && heft package-solution --production' }),
  new FN021005_PKG_scripts_test({ script: 'gulp test', add: false }),
  new FN021006_PKG_scripts_clean({ script: 'heft clean' }),
  new FN021007_PKG_scripts_start({ script: 'heft start --clean' }),
  new FN021008_PKG_scripts_eject_webpack({ script: 'heft eject-webpack' }),
  new FN021009_PKG_overrides_rushstack_heft({ version: '1.1.2' }),
