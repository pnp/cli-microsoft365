...
Start with the step-by-step overview in the [Getting Started guide](user-guide/getting-started/overview.mdx). It shows the happy path for installing the CLI, setting it up, and logging in.

To configure the CLI directly from the command line, run the [setup](./cmd/setup) command:

```sh
m365 setup
```

If you want to create the app registration manually, follow [Use your own Microsoft Entra identity](user-guide/using-own-identity.mdx).
...
See the [Getting Started guide](user-guide/getting-started/overview.mdx) and the rest of the [User Guide](user-guide/getting-started/installing-cli.mdx) to learn more about the CLI for Microsoft 365 and its capabilities.
