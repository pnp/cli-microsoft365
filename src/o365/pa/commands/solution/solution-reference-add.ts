...
class PaSolutionReferenceAddCommand extends Command {
  ...
  public get description(): string {
    return 'Adds a reference inside the project in the current directory to the project at \'path\'';
  }
  ...
  public commandAction(cmd: CommandInstance, args: CommandArgs, cb: (err?: any) => void): void {
    try {
      ...
      var cdsProjectFileContent = fs.readFileSync(cdsProjectFilePath, 'utf8');

      var cdsProjectMutator = new CdsProjectMutator(cdsProjectFileContent);
  ...
  public validate(): CommandValidate {
    return (args: CommandArgs): boolean | string => {
      ...
      var existingCdsProjects = this.getCdsProjectFile(process.cwd());
      ...
      var existingSupportedProjects = this.getSupportedProjectFiles(args.options.path);
  ...
  private getCdsProjectFile(rootPath: string): string[] {
    return fs.readdirSync(rootPath).filter(fn => path.extname(fn).toLowerCase() === '.cdsproj').map(entry => path.join(rootPath, entry));
  }
  ...
  Examples:
    ...
    Adds a reference inside the CDS Solution project in the current directory to the PowerApps component framework project at ${chalk.grey('./projects/ExampleProject')}
