...
class GraphGroupSettingRemoveCommand extends GraphCommand {
  ...
  Remarks:
    ...
    To remove a group setting, you have to first connect to the Microsoft
    Graph using the ${chalk.blue(commands.CONNECT)} command.

    If the specified ${chalk.grey('id')} doesn't refer to an existing group setting, you will get
    a ${chalk.grey('Resource does not exist')} error.

  Examples:

    Remove group setting with ID ${chalk.grey('28beab62-7540-4db1-a23f-29a6018a3848')}. Will prompt
    for confirmation before removing the group setting
      ${chalk.grey(config.delimiter)} ${this.name} --id 28beab62-7540-4db1-a23f-29a6018a3848

    Remove group setting with ID ${chalk.grey('28beab62-7540-4db1-a23f-29a6018a3848')} without prompting
    for confirmation
