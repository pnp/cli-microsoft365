...
## Renamed option `uri` to `uris` in `aad app set` command

In version 7 of the CLI for Microsoft 365, we have renamed the option `uri` to `uris` in [aad app set](./cmd/aad/app/app-set.mdx) command. This change aims to be more consistent with other CLI commands where it allows one or more values to be specified in plural form.

### What action do I need to take?

Update your scripts to use the new option `uris` instead of `uri`. Verify that your scripts work as intended.

## Renamed option `scope` to `scopes` in `aad approleassignment add` and `aad approleassignment remove` commands

In version 7 of the CLI for Microsoft 365, we have renamed the option `scope` to `scopes` in [aad approleassignment add](./cmd/aad/approleassignment/approleassignment-add.mdx) and [aad approleassignment remove](./cmd/aad/approleassignment/approleassignment-remove.mdx) commands. This change aims to be more consistent with other CLI commands where it allows one or more values to be specified in plural form.

### What action do I need to take?

Update your scripts to use the new option `scopes` instead of `scope`. Verify that your scripts work as intended.

## Renamed option `changeType` to `changeTypes` in `graph subscription add` and `aad approleassignment remove` commands

In version 7 of the CLI for Microsoft 365, we have renamed the option `changeType` to `changeTypes` in [graph subscription add](./cmd/graph/subscription/subscription-add.mdx) command. This change aims to be more consistent with other CLI commands where it allows one or more values to be specified in plural form.

### What action do I need to take?

Update your scripts to use the new option `changeTypes` instead of `changeType`. Verify that your scripts work as intended.

## Renamed options in `spo group member add` command

In version 7 of the CLI for Microsoft 365, we have renamed the below options in [spo group member add](./cmd/spo/group/group-member-add.mdx) command. 

- Option `userName` is renamed to `userNames`
- Option `email` is renamed to `emails`
- Option `userId` is renamed to `userIds`
- Option `aadGroupId` is renamed to `aadGroupIds`

This change aims to be more consistent with other CLI commands where it allows one or more values to be specified in plural form.

### What action do I need to take?

Update your scripts to use the new options. Verify that your scripts work as intended.

## Renamed options in `teams channel member add` command

In version 7 of the CLI for Microsoft 365, we have renamed the below options in [teams channel member add](./cmd/teams/channel/channel-member-add.mdx) command. 

- Option `userId` is renamed to `userIds`
- Option `userDisplayName` is renamed to `userDisplayNames`
