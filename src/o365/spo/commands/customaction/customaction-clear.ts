...
class SpoCustomActionClearCommand extends SpoCommand {
  ...
  public commandAction(cmd: CommandInstance, args: CommandArgs, cb: () => void): void {
    const clearCustomActions = (): void => {
      ...
      auth
        ...
        .then((contextResponse: ContextInfo): Promise<void> => {
          if (this.debug) {
            cmd.log('Response:');
  ...
  Examples:
    ...
    Clears all user custom actions for both site and site collection ${chalk.grey('https://contoso.sharepoint.com/sites/test')}
      ${chalk.grey(config.delimiter)} ${commands.CUSTOMACTION_CLEAR} -u https://contoso.sharepoint.com/sites/test --confirm

    Clears all user custom actions for site ${chalk.grey('https://contoso.sharepoint.com/sites/test')}
      ${chalk.grey(config.delimiter)} ${commands.CUSTOMACTION_CLEAR} -u https://contoso.sharepoint.com/sites/test -s Web

    Clears all user custom actions for site collection ${chalk.grey('https://contoso.sharepoint.com/sites/test')}
  ...
  More information:
    ...
    UserCustomAction REST API resources:
      https://msdn.microsoft.com/en-us/library/office/dn531432.aspx#bk_UserCustomActionCollectionClear
      `);
  }
}
