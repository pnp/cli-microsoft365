...
class AadAppDeleteCommand extends GraphCommand {
  ...
  private processAppDelete(logger: Logger, args: CommandArgs): Promise<void> {
    return this
      .getAadApp(args, logger)
      .then((aadAppId: string): Promise<void> => {
        if (this.verbose) {
  ...
  private getAadApp(args: CommandArgs, logger: Logger): Promise<string> {
    ...
    return request
      ...
      .then((res: { value: { id: string }[] }): Promise<string> => {
        ...
        return Promise.reject(`Multiple Azure AD application registration with name ${name} found. Please disambiguate using app object IDs: ${res.value.map(a => a.id).join(', ')}`);
      });
  }
