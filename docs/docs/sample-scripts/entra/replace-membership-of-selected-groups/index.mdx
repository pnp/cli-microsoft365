      ...
      $users | Where-Object { $_.userPrincipalName -eq $oldUser } | ForEach-Object {
        ...
        if ($isMember -or $isTeam) {
          try {
            ...
            m365 entra m365group user add --groupId $groupId --userName $newUser | Get-CLIValue
          }
          catch {
        ...
        if ($isOwner) {
          try {
            ...
            m365 entra m365group user add --groupId $groupId --userName $newUser --role Owner | Get-CLIValue
          }
          catch {
            Write-Host $_.Exception.Message -ForegroundColor White
          }
