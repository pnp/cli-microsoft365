...
describe(commands.LIST_VIEW_REMOVE, () => {
  ...
  it('fails validation if the webUrl option is not a valid SharePoint site URL', async () => {
    const actual = await command.validate({ options: { webUrl: 'foo', viewId: viewId, listId: listId } }, commandInfo);
    assert.notStrictEqual(actual, true);
  });

  it('fails validation if the listId option is not a valid GUID', async () => {
    const actual = await command.validate({ options: { webUrl: webUrl, listId: '12345', viewId: viewId } }, commandInfo);
    assert.notStrictEqual(actual, true);
  });

  it('fails validation if the viewId option is not a valid GUID', async () => {
    const actual = await command.validate({ options: { webUrl: webUrl, listId: listId, viewId: '12345' } }, commandInfo);
    assert.notStrictEqual(actual, true);
  });

  it('passes validation if valid options are specified', async () => {
    const actual = await command.validate({ options: { webUrl: webUrl, listTitle: listTitle, viewTitle: viewTitle } }, commandInfo);
  ...
  it('prompts before removing the specified view from list when confirm option not passed', async () => {
  ...
  it('removes view from the list using viewId and listUrl when prompt confirmed (debug)', async () => {
