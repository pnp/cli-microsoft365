...
describe(commands.TEAMS_REPORT_PSTNCALLS, () => {
  ...
  it('get pstncalls in teams with no toDateTime specified', (done) => {
  ...
  it('get pstncalls in teams output in json', (done) => {
    ...
    command.action(logger, { options: { debug: false, fromDateTime: '2019-11-01', toDateTime: '2019-12-01', output: 'json' } }, () => {
      try {
        assert.strictEqual(requestStub.lastCall.args[0].url, "https://graph.microsoft.com/beta/communications/callRecords/getPstnCalls(fromDateTime=2019-11-01,toDateTime=2019-12-01)");
        assert.strictEqual(requestStub.lastCall.args[0].headers["accept"], 'application/json;odata.metadata=none');
        done();
