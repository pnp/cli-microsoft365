...
class TenantServiceHealthListCommand extends GraphCommand {
  public get name(): string {
    return commands.SERVICE_HEALTH_LIST;
  }
  ...
  private listServiceHealth(options: Options): Promise<ServiceHealth[]> {
    const requestOptions: any = {
      url: `${this.resource}/v1.0/admin/serviceAnnouncement/healthOverviews${options.issues ? '?$expand=issues' : ''}`,
      headers: {
...
module.exports = new TenantServiceHealthListCommand();