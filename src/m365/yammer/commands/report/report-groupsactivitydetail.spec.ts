...
describe(commands.YAMMER_REPORT_GROUPSACTIVITYDETAIL, () => {
  ...
  it('has correct name', () => {
    assert.strictEqual(command.name.startsWith(commands.YAMMER_REPORT_GROUPSACTIVITYDETAIL), true);
  });
  ...
  it('gets the report for the last week', (done) => {
    ...
    cmdInstance.action({ options: { debug: false, period: 'D7' } }, () => {
      try {
        assert.strictEqual(requestStub.lastCall.args[0].url, "https://graph.microsoft.com/v1.0/reports/getYammerGroupsActivityDetail(period='D7')");
