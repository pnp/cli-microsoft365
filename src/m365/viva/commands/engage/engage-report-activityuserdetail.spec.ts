...
import command from './engage-report-activityuserdetail.js';
import yammerCommands from './yammerCommands.js';
import { cli } from '../../../../cli/cli.js';

describe(commands.ENGAGE_REPORT_ACTIVITYUSERDETAIL, () => {
  let log: string[];
  let logger: Logger;
  ...
  it('has correct name', () => {
    assert.strictEqual(command.name, commands.ENGAGE_REPORT_ACTIVITYUSERDETAIL);
  });
  ...
  it('defines correct alias', () => {
    ...
    assert.deepStrictEqual(alias, [yammerCommands.REPORT_ACTIVITYUSERDETAIL]);
  });
  ...
  it('gets the report for the last week', async () => {
    ...
    assert.strictEqual(requestStub.lastCall.args[0].url, "https://graph.microsoft.com/v1.0/reports/getYammerActivityUserDetail(period='D7')");
