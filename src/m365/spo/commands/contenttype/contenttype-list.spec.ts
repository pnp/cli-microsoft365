...
describe(commands.CONTENTTYPE_LIST, () => {
  ...
  it('passes validation if the category option is defined', () => {
    const actual = command.validate({ options: { webUrl: 'https://contoso.sharepoint.com', category: 'List Content Types' } });
    assert(actual);
  });
  ...
  it('retrieves all content types by category (debug)', (done) => {
    sinon.stub(request, 'get').callsFake((opts) => {
      if ((opts.url as string).indexOf(`https://contoso.sharepoint.com/sites/test/_api/web/ContentTypes?$filter=Group eq 'List Content Types'`) > -1) {
        if (opts.headers &&
          opts.headers.accept &&
          opts.headers.accept.indexOf('application/json') === 0) {
          return Promise.resolve({
