...
Before you can use CLI for Microsoft 365, you must have a Microsoft Entra application registration that the CLI can use to authenticate to your tenant. The easiest path is to run [m365 setup](../cmd/setup.mdx), but if you cannot use the setup wizard, follow this guide to create and configure the app registration manually.

The `m365 setup` command can create an application registration with the required permissions for common CLI scenarios, including permissions that require administrative consent. This makes it easy to get started and try commands across multiple CLI for Microsoft 365 workloads without manually managing permissions in Azure.
...
In production environments, administrators will often want to provide their own Microsoft Entra application registration to use with the CLI so they can control exactly which permissions are granted.
