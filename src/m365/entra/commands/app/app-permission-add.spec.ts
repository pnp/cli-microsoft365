...
describe(commands.APP_PERMISSION_ADD, () => {
  ...
  it('throws an error when application specified by appObjectId is not found', async () => {
    sinon.stub(entraApp, 'getAppRegistrationByObjectId').callsFake(async (objectId: string) => {
      throw `App with objectId '${objectId}' not found in Microsoft Entra ID`;
    });
    ...
    await assert.rejects(command.action(logger, { options: commandOptionsSchema.parse({ appObjectId: appObjectId, applicationPermissions: applicationPermissions, verbose: true }) }),
      new CommandError(`App with objectId '${appObjectId}' not found in Microsoft Entra ID`));
  });
  ...
  it('handles error when multiple apps with the specified name found', async () => {
    ...
    }), new CommandError(`Multiple apps with name 'My App' found in Microsoft Entra ID. Found: 9b1b1e42-794b-4c71-93ac-5ed92488b67f, 9b1b1e42-794b-4c71-93ac-5ed92488b67g.`));
  });
