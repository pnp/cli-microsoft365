...
class TenantReportActiveUserDetailCommand extends DateAndPeriodBasedReport {
  ...
  public get description(): string {
    return 'Gets details about Office 365 active users';
  }
  ...
  Examples:
    ...
    and exports the report data in the specified path in text format
      ${commands.TENANT_REPORT_ACTIVEUSERDETAIL} --period D7 --output text --outputFile "activeuserdetail.txt"

    Gets details about Office 365 active users for the last week
    and exports the report data in the specified path in json format
      ${commands.TENANT_REPORT_ACTIVEUSERDETAIL} --period D7 --output json --outputFile "activeuserdetail.json"
  `);
  }
}
