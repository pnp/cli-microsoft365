...
class GraphSiteClassificationDisableCommand extends GraphCommand {
  ...
  public get description(): string {
    return 'Disables site classification configuration';
  }
  ...
  public commandAction(cmd: CommandInstance, args: CommandArgs, cb: (err?: any) => void): void {
    const disableSiteClassification: () => void = (): void => {
      auth
        ...
        .then((res: DirectorySettingTemplatesRsp): request.RequestPromise | Promise<void> => {
          ...
          if (unifiedGroupSetting[0] == null || unifiedGroupSetting[0].id == null || unifiedGroupSetting[0].id.length === 0) {
            cb(new CommandError("Missing UnifiedGroupSettting id"));
            return Promise.reject();
          }
    ...
    if (args.options.confirm) {
      ...
      cmd.prompt({
        ...
        name: 'continue',
        default: false,
        message: `Are you sure you want to disable site classification configuration ?`,
  ...
  public options(): CommandOption[] {
    const options: CommandOption[] = [
      {
        option: '--confirm',
        description: 'Don\'t prompt for confirming disabling site classification configuration'
      }
    ];
