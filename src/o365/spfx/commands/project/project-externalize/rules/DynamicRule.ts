...
interface IScriptCheckAPIResponse {
...
export class DynamicRule extends BasicDependencyRule {
  ...
  private pathContainsMinifySuffix(path: string): boolean {
    return this.fileVariationSuffixes.map(y => path.indexOf(y))
  ...
  private getExternalEntryForFilePath(filePath: string, packageName: string, version: string): Promise<ExternalizeEntry | undefined> {
    ...
    return this.testUrl(url).then((testResult) => {
  ...
  private getModuleType(url: string): Promise<IScriptCheckAPIResponse> {
    return request
      .post<IScriptCheckAPIResponse>({
  ...
  private testUrl(url: string): Promise<boolean> {
    return request.head({ url: url, headers: { 'x-anonymous': 'true' } }).then(() => true).catch(() => false);
  }
  ...
  private getFilePath(packageName: string): string[] {
    ...
    try {
      ...
      return filesPaths.filter((x, idx) => filesPaths.indexOf(x) === idx) //filtering duplicates
              .map(x => x.endsWith('.js') ? x : `${x}.js`); //some package managers ommit the file extension
    }
    catch {
      return [];// file doesn't exist, giving up
    }
  }
