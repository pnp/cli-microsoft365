...
class SpoEventreceiverRemoveCommand extends SpoCommand {
  ...
  public async commandAction(logger: Logger, args: CommandArgs): Promise<void> {
    if (args.options.force) {
      ...
      const result = await Cli.promptForConfirmation(`Are you sure you want to remove event receiver with ${args.options.id ? `id ${args.options.id}` : `name ${args.options.name}`}?`);
