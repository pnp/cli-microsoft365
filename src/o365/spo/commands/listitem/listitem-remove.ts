...
class SpoListItemRemoveCommand extends SpoCommand {
  ...
  public commandAction(cmd: CommandInstance, args: CommandArgs, cb: () => void): void {
      ...
      auth
        ...
        .then((accessToken: string): request.RequestPromise => {
          ...
          if (args.options.listId) {
            requestUrl = `${args.options.webUrl}/_api/web/lists(guid'${encodeURIComponent(args.options.listId as string)}')`;
          }
          else {
    ...
    if (args.options.confirm) {
      ...
      cmd.prompt({
        ...
        name: 'continue',
        default: false,
        message: `Are you sure you want to ${args.options.recycle? "recycle": "remove"} the list item ${args.options.id} from list ${args.options.listId || args.options.listTitle} located at site ${args.options.webUrl}?`,
  ...
  public options(): CommandOption[] {
    const options: CommandOption[] = [
      ...
      },
      {
        ...
        description: 'The ID of the list to remove the item from. Specify either id or title but not both'
      },
      {
        option: '-t, --listTitle [listTitle]',
        description: 'Title of the list to remove the item from. Specify either id or title but not both'
      },
      {
        option: '--recycle',
        description: 'Recycle the item instead of actually deleting it'
      },
      {
        option: '--confirm',
  ...
  Remarks:
    ...
    To remove a list item, you have to first connect to SharePoint using the
    ${chalk.blue(commands.CONNECT)} command, eg. ${chalk.grey(`${config.delimiter} ${commands.CONNECT} https://contoso.sharepoint.com`)}.
        
  Examples:
  
    Remove the list item with ID ${chalk.grey(1)} located in site
    ${chalk.grey('https://contoso.sharepoint.com/sites/project-x')} at list ${chalk.grey('0cd891ef-afce-4e55-b836-fce03286cccf')}
    ...
    Remove the list item with with ID ${chalk.grey(1)} located in site
    ${chalk.grey('https://contoso.sharepoint.com/sites/project-x')} at list with title ${chalk.grey('List 1')}
