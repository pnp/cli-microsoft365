...
describe(commands.TERM_GET, () => {
  ...
  it('fails validation if neither id nor name specified', async () => {
    const actual = await command.validate({ options: { termGroupName: 'People', termSetName: 'Department' } }, commandInfo);
    assert.notStrictEqual(actual, true);
  });

  it('fails validation if both id and name specified', async () => {
    const actual = await command.validate({ options: { id: '9e54299e-208a-4000-8546-cc4139091b26', name: 'IT', termGroupName: 'People', termSetName: 'Department' } }, commandInfo);
    assert.notStrictEqual(actual, true);
  });
  ...
  it('fails validation when only name specified', async () => {
    const actual = await command.validate({ options: { name: 'IT' } }, commandInfo);
    assert.notStrictEqual(actual, true);
  });

  it('fails validation when only name and termGroupId specified', async () => {
    const actual = await command.validate({ options: { name: 'IT', termGroupId: '9e54299e-208a-4000-8546-cc4139091b26' } }, commandInfo);
    assert.notStrictEqual(actual, true);
  });

  it('fails validation if both termGroupId and termGroupName specified', async () => {
    const actual = await command.validate({ options: { name: 'IT', termGroupId: '9e54299e-208a-4000-8546-cc4139091b27', termGroupName: 'People', termSetName: 'Department' } }, commandInfo);
    assert.notStrictEqual(actual, true);
  });
  ...
  it('fails validation if both termSetId and termSetName specified', async () => {
    const actual = await command.validate({ options: { name: 'IT', termGroupId: '9e54299e-208a-4000-8546-cc4139091b27', termSetName: 'Department', termSetId: '9e54299e-208a-4000-8546-cc4139091b2a' } }, commandInfo);
    assert.notStrictEqual(actual, true);
  });

  it('fails validation if termSetId is not a valid GUID', async () => {
    const actual = await command.validate({ options: { name: 'IT', termSetId: 'invalid', termGroupName: 'People' } }, commandInfo);
    assert.notStrictEqual(actual, true);
  });

  it('passes validation when name, termGroupName and termSetName specified', async () => {
    const actual = await command.validate({ options: { name: 'IT', termGroupName: 'People', termSetName: 'Department' } }, commandInfo);
    assert.strictEqual(actual, true);
  });
