...
const tenantCdnGetCommand: Command = require('./tenant-cdn-get');
import * as assert from 'assert';
import * as request from 'request-promise-native';
import Utils from '../../../Utils';

describe(commands.TENANT_CDN_GET, () => {
  let vorpal: Vorpal;
  let log: string[];
  let cmdInstance: any;
  let trackEvent: any;
  ...
  it('has correct name', () => {
    assert.equal(tenantCdnGetCommand.name.startsWith(commands.TENANT_CDN_GET), true);
  });

  it('has a description', () => {
    assert.notEqual(tenantCdnGetCommand.description, null);
  });
  ...
  it('supports verbose mode', () => {
    const options = (tenantCdnGetCommand.options() as CommandOption[]);
  ...
  it('accepts Public SharePoint Online CDN type', () => {
    const actual = (tenantCdnGetCommand.validate() as CommandValidate)({ options: { type: 'Public' } });
    assert(actual);
  });

  it('accepts Private SharePoint Online CDN type', () => {
    const actual = (tenantCdnGetCommand.validate() as CommandValidate)({ options: { type: 'Private' } });
    assert(actual);
  });

  it('rejects invalid SharePoint Online CDN type', () => {
    const type = 'foo';
    const actual = (tenantCdnGetCommand.validate() as CommandValidate)({ options: { type: type } });
  ...
  it('doesn\'t fail validation if the optional type option not specified', () => {
    const actual = (tenantCdnGetCommand.validate() as CommandValidate)({ options: {} });
    assert(actual);
  });
  ...
  it('has help referring to the right command', () => {
    ...
    (tenantCdnGetCommand.help() as CommandHelp)({}, helpLog);
    assert(find.calledWith(commands.TENANT_CDN_GET));
  });
  ...
  it('has help with examples', () => {
    ...
    (tenantCdnGetCommand.help() as CommandHelp)({}, log);
