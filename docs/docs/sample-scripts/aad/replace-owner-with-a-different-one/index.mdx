...
<Tabs>
  ...
  function Replace-Owner {
      ...
      $groupsToProcess = m365 aad m365group list | Get-CLIValue  
      $i = 0
      $groupsToProcess | ForEach-Object {
          ...
          $hasOwner = m365 aad m365group user list --groupId $group.id --query "[?userType=='Owner' && userPrincipalName=='$oldUser'].[id]" | Get-CLIValue
          if ($hasOwner -ne $null) {
              Write-Host "Found $oldUser" -ForegroundColor Green
              try {
                  Write-Host "Granting $newUser owner rights"
                  m365 aad m365group user add --groupId $group.id --userName $newUser --role Owner | Get-CLIValue
              }
              catch  {
              ...
              try {
                  ...
                  m365 aad m365group user remove --groupId $group.id --userName $oldUser | Get-CLIValue
              }
              catch  {
