...
class SpoFileSharingLinkListCommand extends SpoCommand {
  ...
  public async commandAction(logger: Logger, args: CommandArgs): Promise<void> {
    ...
    try {
      ...
      const requestUrl = `https://graph.microsoft.com/v1.0/sites/${fileDetails.SiteId}/drives/${fileDetails.VroomDriveID}/items/${fileDetails.VroomItemID}/permissions?$filter=Link ne null`;
      ...
      if (!args.options.output || args.options.output === 'json' || args.options.output === 'md') {
        ...
        logger.log(sharingLinks.map((i: any) => {
          return {
            id: i.id,
            roles: i.roles.join(','),
            link: i.link.webUrl
