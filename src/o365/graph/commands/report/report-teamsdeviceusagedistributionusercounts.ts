...
class GraphReportTeamsDeviceUsageDistributionUserCounts extends GraphCommand {
  public get name(): string {
    return `${commands.REPORT_TEAMSDEVICEUSAGEDISTRIBUTIONUSERCOUNTS}`;
  }
  ...
  public commandAction(cmd: CommandInstance, args: CommandArgs, cb: () => void): void {
    auth
      ...
      .then((): Promise<{}> => {
        const endpoint: string = `${auth.service.resource}/v1.0/reports/getTeamsDeviceUsageDistributionUserCounts(period='${encodeURIComponent(args.options.period)}')`;
  ...
  Remarks:
    ...
    To get the number of Microsoft Teams unique users by device type, you have to first
  ...
  Examples:
    ...
    Gets the number of Microsoft Teams unique users by device type for the last week
      ${chalk.grey(config.delimiter)} ${commands.REPORT_TEAMSDEVICEUSAGEDISTRIBUTIONUSERCOUNTS} --period 'D7'
`);
  }
}

module.exports = new GraphReportTeamsDeviceUsageDistributionUserCounts();