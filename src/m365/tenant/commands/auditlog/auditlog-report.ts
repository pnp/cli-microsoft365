...
class TenantAuditlogReportCommand extends Command {
  ...
  public commandAction(logger: Logger, args: CommandArgs, cb: (err?: any) => void): void {
    ...
    this.getCompleteAuditReports(args, logger)
    ...
    return request.get<ActivityfeedSubscription[]>(requestOptions)
      .then((subscriptionLists: ActivityfeedSubscription[]): boolean => {
        return subscriptionLists.some(subscriptionList => subscriptionList
          .contentType === (<any>AuditContentTypes)[args.options.contentType] && subscriptionList.status === 'enabled');
      })
  ...
  public validate(args: CommandArgs): boolean | string {
    ...
    if (args.options.startTime || args.options.endTime) {
      if (!args.options.startTime) {
        return `Please specify a starttime`;
      }
