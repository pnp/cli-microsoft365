...
Breaking down deltas to just two SPFx versions allows users to upgrade their projects in any supported SPFx range, eg. 1.4.1 to 1.16.0 or 1.14.0 to the latest SPFx version supported by the `spfx project upgrade` command.
...
A rule can supersede one or more other rules. You define superseded rules, by overriding the `supersedes` getter in the rule and return the array of rule IDs that the rule supersedes, eg.
...
![Diagram illustrating the process of the spfx project upgrade command](../../images/how-it-works/spfx-project-upgrade-process.png)
...
Once the command located the SPFx project's root folder, it will try to determine the project's version. It does this by using the following steps:
...
After it determined the project version, the command will verify that it supports the specified version, and the specified version to upgrade the project to is newer than the detected project version.
...
After the command built the list of rules to upgrade the project to the specified SPFx version, it executes them in the order they're defined in deltas.
