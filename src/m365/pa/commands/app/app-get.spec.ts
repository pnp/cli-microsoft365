...
describe(commands.APP_GET, () => {
  ...
  it(`retrieves information about the specified app using displayName`, async () => {
    ...
    sinon.stub(pa, 'getAppByDisplayName').resolves(apps[2]);
  ...
  it('correctly handles App not found using displayName (debug)', async () => {
    sinon.stub(pa, 'getAppByDisplayName').rejects(new Error(`No app found with displayName 'NoAppFound'`));
  ...
  it('correctly handles no apps found using displayName (debug)', async () => {
    sinon.stub(pa, 'getAppByDisplayName').rejects(new Error('No apps found'));

    await assert.rejects(command.action(logger, { options: { debug: true, displayName: 'Playwright' } }),
