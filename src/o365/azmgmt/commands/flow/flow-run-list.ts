...
class AzmgmtFlowRunListCommand extends AzmgmtCommand {
  ...
  public commandAction(cmd: CommandInstance, args: CommandArgs, cb: () => void): void {
    auth
      ...
      .then((accessToken: string): request.RequestPromise => {
        ...
        if (this.verbose) {
          cmd.log(`Retrieving list of runs for Microsoft Flow ${args.options.name}...`);
        }

        const requestOptions: any = {
          url: `${auth.service.resource}providers/Microsoft.ProcessSimple/environments/${encodeURIComponent(args.options.environment)}/flows/${encodeURIComponent(args.options.name)}/runs?api-version=2016-11-01`,
  ...
  Examples:
    ...
    List runs of the specified Microsoft Flow
      ${chalk.grey(config.delimiter)} ${this.getCommandName()} --environment Default-d87a7535-dd31-4437-bfe1-95340acd55c5 --name 5923cb07-ce1a-4a5c-ab81-257ce820109a
`);
  }
}
