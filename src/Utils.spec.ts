...
describe('Utils', () => {
  ...
  it('isPositiveInt returns true if positive integer', () => {
    const result = Utils.isPositiveInt("10");
    assert.equal(result, true);
  });

  it('isPositiveInt returns false if negative number', () => {
    const result = Utils.isPositiveInt("-10");
    assert.equal(result, false);
  });

  it('isPositiveInt returns false if blank string passed', () => {
    const result = Utils.isPositiveInt("");
    assert.equal(result, false);
  });

  it('isPositiveInt returns false if alpha characters passed', () => {
    const result = Utils.isPositiveInt("sharing is caring!");
    assert.equal(result, false);
  });

  it('isPositiveInt returns false if decimal passed', () => {
    const result = Utils.isPositiveInt("sharing is caring!");
    assert.equal(result, false);
  });
