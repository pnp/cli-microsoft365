...
export const teams = {
   ...
   * @throws Error when multiple teams with the same name were found or prompts for a choice.
  ...
  async getChannelIdByDisplayName(teamId: string, name: string): Promise<string> {
    ...
    if (!channelItem) {
      throw Error(`The specified channel ${name} does not exist in the Microsoft Teams team ${teamId}.`);
    }
