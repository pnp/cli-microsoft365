...
class SpoSiteAddCommand extends SpoCommand {
  ...
  #initTelemetry(): void {
    this.telemetry.push((args: CommandArgs) => {
      ...
      telemetryProps.withAppCatalog = args.options.withAppCatalog;
  ...
  public async commandAction(logger: Logger, args: CommandArgs): Promise<void> {
    ...
    if (isClassicSite) {
      ...
      siteUrl = await this.createModernSite(logger, args);
    }
  ...
  private async createModernSite(logger: Logger, args: CommandArgs): Promise<string> {
    ...
    try {
      ...
      if (isTeamSite) {
        ...
        if (response.SiteStatus === 2) {
          // Continue to set siteUrl and log
        }
        else {
