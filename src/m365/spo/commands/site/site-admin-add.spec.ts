...
describe(commands.SITE_ADMIN_ADD, () => {
  ...
  it('correctly handles incorrect site Id guid in admin mode', async () => {
    ...
    await assert.rejects(command.action(logger, { options: { siteUrl: siteUrl, userId: adminToAddId, asAdmin: true } }), new CommandError('Failed to obtain site Id from the provided site URL.'));
  });
  ...
  it('correctly handles error when site id is not found for specified site URL in admin mode', async () => {
    ...
    await assert.rejects(command.action(logger, { options: { siteUrl: siteUrl, userId: adminToAddId, asAdmin: true } }), new CommandError('Unknown Error'));
  });
