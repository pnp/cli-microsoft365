...
describe(commands.LIST_ROLEINHERITANCE_BREAK, () => {
  ...
  it('aborts breaking role inheritance when prompt not confirmed', (done) => {
  ...
  it('prompts before breaking role inheritance when confirmation argument not passed (Title)', (done) => {
    command.action(logger, {
      options: {
        debug: true,
        webUrl: 'https://contoso.sharepoint.com',
  ...
  it('prompts before breaking role inheritance when confirmation argument not passed (id)', (done) => {
    command.action(logger, {
      options: {
        debug: true,
        webUrl: 'https://contoso.sharepoint.com',
  ...
  it('break role inheritance when prompt confirmed', (done) => {
    sinon.stub(request, 'post').callsFake((opts) => {
      if ((opts.url as string).indexOf('/_api/web/lists/getbytitle(\'test\')/breakroleinheritance(true)') > -1) {
        return Promise.resolve();
      }
