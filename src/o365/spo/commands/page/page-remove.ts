...
class SpoPageRemoveCommand extends SpoCommand {
  ...
  Remarks:
    ...
    To remove a modern page, you have to first connect to a SharePoint site using the ${chalk.blue(
		commands.CONNECT
	)} command,
  ...
  Examples:
    ...
    ${chalk.grey(config.delimiter)} ${this
				.name} --name page.aspx --webUrl https://contoso.sharepoint.com/sites/a-team --confirm
    `
		);
	}
}
