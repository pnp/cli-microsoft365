...
describe(commands.FOLDER_REMOVE, () => {
  ...
  it('fails validation if the webUrl option not specified', () => {
    const actual = (command.validate() as CommandValidate)({ options: { sourceUrl: '/Shared Documents' } });
    assert.notEqual(actual, true);
  });

  it('fails validation if the webUrl option is not a valid SharePoint site URL', () => {
    const actual = (command.validate() as CommandValidate)({ options: { webUrl: 'foo', sourceUrl: '/Shared Documents' } });
    assert.notEqual(actual, true);
  });
