...
Adds the specified application and/or delegated permissions to a specified Entra application registration
...
```md definition-list
`-i, --appId [appId]`
: Client ID of the Entra application registration to add the API permissions to. Specify either `appId`, `appName` or `appObjectId`.
...
: Object ID of the Entra application registration to add the API permissions to. Specify either `appId`, `appName` or `appObjectId`.
...
Grant multiple delegated API permissions to an Entra application registration specified by client id.
...
Grant multiple delegated API permissions to an Entra application registration specified by object id.
...
Grant multiple app-only permissions to an Entra application registration specified by app display name and grant admin consent.
