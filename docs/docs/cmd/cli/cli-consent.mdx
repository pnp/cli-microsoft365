...
Consent additional permissions for the Microsoft Entra ID application used by the CLI for Microsoft 365
...
Using the `cli consent` command you can consent additional permissions for the Microsoft Entra ID application used by the CLI for Microsoft 365. This is for example necessary to use Viva Engage commands, which require the Viva Engage API permission that isn't granted to the CLI by default.
