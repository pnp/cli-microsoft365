...
describe(commands.SITE_SET, () => {
  ...
  it('sets sharingCapabilities for Site - Disabled', (done) => {
    ...
    cmdInstance.action({ options: { debug: false, sharingCapabilities: 'Disabled', url: 'https://contoso.sharepoint.com/sites/Sales' } }, () => {
      try {
        assert(cmdInstanceLogSpy.notCalled);
        done();
      }
  ...
  it('sets sharingCapabilities for Site - (Debug) -  Disabled', (done) => {
    ...
    cmdInstance.action({ options: { debug: true, sharingCapabilities: 'Disabled', url: 'https://contoso.sharepoint.com/sites/Sales' } }, () => {
      try {
  ...
  it('correctly handles API error while updating sharingCapabilities properties', (done) => {
    sinon.stub(request, 'get').callsFake((opts) => {
      if (opts.url === 'https://contoso.sharepoint.com/sites/Sales/_api/site?$select=GroupId,Id') {
        return Promise.resolve({
          Id: '255a50b2-527f-4413-8485-57f4c17a24d1',
    ...
    cmdInstance.action({ options: { debug: false, url: 'https://contoso.sharepoint.com/sites/Sales', sharingCapabilities: 'Invalid'} }, (err?: any) => {
      try {
        assert.equal(JSON.stringify(err), JSON.stringify(new CommandError("An error has occurred.")));
        done();
      }
  ...
  it('passes validation if non existing sharingCapabilities specified', () => {
    const sharingCapabilityvalue = 'nonExistentSharingCapabilityValue';
    const actual = (command.validate() as CommandValidate)({ options: { url: 'https://contoso.sharepoint.com', sharingCapabilities: sharingCapabilityvalue } });
    assert.notEqual(actual, true);
  });

  it('passes validation if correct sharingCapabilities specified', () => {
