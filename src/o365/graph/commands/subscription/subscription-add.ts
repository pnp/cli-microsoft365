...
class GraphSubscriptionAddCommand extends GraphCommand {
  ...
  private getExpirationDateTimeOrDefault(cmd: CommandInstance, args: CommandArgs): string {
    ...
    const perResource: any = DEFAULT_EXPIRATION_DELAY_IN_MINUTES_PER_RESOURCE_TYPE;
    for (let resource in perResource) {
      if (args.options.resource.indexOf(resource) < 0) {
        continue;
      }
      const resolvedExpirationDelay = perResource[resource] as number;
  ...
  public validate(): CommandValidate {
    return (args: CommandArgs): boolean | string => {
      ...
      if (args.options.clientState && args.options.clientState.length > 128) {
        return 'The clientState value exceeds the maximum length (128 characters)';
      }
  ...
  public commandHelp(args: {}, log: (help: string) => void): void {
    ...
    Notifications are sent for the requested types of changes on the subscribed folder,
    or any file, folder, or other ${chalk.grey(`driveItem`)} instances in its hierarchy.
    You cannot subscribe to ${chalk.grey(`drive`)} or ${chalk.grey(`driveItem`)} instances that are not folders, such as individual files.
    ...
    - Do not use the Outlook sharing permissions (Contacts.Read.Shared, Calendars.Read.Shared, Mail.Read.Shared, and their read/write counterparts),
    as they do not support subscribing to change notifications on items in shared or delegated folders.
