...
function extend(target: any, source: any, noOverwrite = false): any {
	...
	return Object.getOwnPropertyNames(source).filter((v: string) => check(target, v)).reduce((t: any, v: string) => {
		t[v] = source[v];
		return t;
	}, target);
}
 ...
 */
function reindex(
	collection?: { order: number; columns?: { order: number }[]; controls?: { order: number }[] }[]
): void {
	if (!collection) {
		return;
	}
...
export class ClientSidePage {
   ...
   */
	constructor(public sections: CanvasSection[] = []) {}
   ...
   */
	private mergePartToTree(control: ClientSidePart): void {
		...
		const sections = this.sections.filter((s) => s.order === controlData.position.zoneIndex);
		if (sections.length < 1) {
			section = new CanvasSection(this, controlData.position.zoneIndex);
			this.sections.push(section);
		} else {
			section = sections[0];
		}

		const columns = section.columns.filter((c) => c.order === controlData.position.sectionIndex);
		if (columns.length < 1) {
			column = new CanvasColumn(section, controlData.position.sectionIndex, controlData.position.sectionFactor);
   ...
   */
	private mergeColumnToTree(column: CanvasColumn): void {
		let section: CanvasSection | null = null;
		const controlData = column.controlData as ClientSideControlData;
		const sections = this.sections.filter((s) => s.order === controlData.position.zoneIndex);
...
export abstract class CanvasControl {
   ...
   */
	...
	public fromHtml(html: string): void {
		this.controlData = ClientSidePage.escapedStringToJson<ClientSideControlData>(getAttrValueFromString(
			html,
			'data-sp-controldata'
		) as string);
		this.dataVersion = getAttrValueFromString(html, 'data-sp-canvasdataversion');
		this.controlType = this.controlData.controlType;
		this.id = this.controlData && this.controlData.id;
	}
...
export class CanvasColumn extends CanvasControl {
	...
	public insertControl(control: ClientSidePart, index?: number): this {
		if ((!index && index != 0) || index >= this.controls.length) {
	...
	public toHtml(): string {
		...
		if (this.controls.length < 1) {
			html.push(
				`<div data-sp-canvascontrol="" data-sp-canvasdataversion="${this
					.dataVersion}" data-sp-controldata="${this.jsonData}"></div>`
			);
		} else {
	...
	public getControlData(): ClientSideControlData {
		return {
			...
			position: {
				...
				zoneIndex: (this.section && this.section.order) as number
			}
		};
	}
   ...
   */
	public remove(): void {
		const section = this.section as CanvasSection;
		section.columns = section.columns.filter((column) => column.id !== this.id);
		reindex((this.column as CanvasColumn).controls);
	}
}
...
export abstract class ClientSidePart extends CanvasControl {
   ...
   */
	public remove(): void {
		(this.column as CanvasColumn).controls = (this.column as CanvasColumn).controls.filter(
			(control) => control.id !== this.id
		);
		reindex((this.column as CanvasColumn).controls);
	}
}
...
export class ClientSideText extends ClientSidePart {
   ...
   */
	...
	public getControlData(): ClientSideControlData {
		const column = this.column as CanvasColumn;
		return {
			...
			position: {
				...
				sectionFactor: column.factor,
				sectionIndex: column.order,
				zoneIndex: (column.section as CanvasSection).order
			}
		};
	}
	...
	public toHtml(index: number): string {
		...
		html.push(
			`<div data-sp-canvascontrol="" data-sp-canvasdataversion="${this.dataVersion}" data-sp-controldata="${this
				.jsonData}">`
		);
...
export class ClientSideWebpart extends ClientSidePart {
	...
	public toHtml(index: number): string {
		...
		html.push(
			`<div data-sp-canvascontrol="" data-sp-canvasdataversion="${this
				.canvasDataVersion}" data-sp-controldata="${this.jsonData}">`
		);

		html.push(
			`<div data-sp-webpart="" data-sp-webpartdataversion="${this
				.dataVersion}" data-sp-webpartdata="${ClientSidePage.jsonToEscapedString(data)}">`
		);
	...
	public fromHtml(html: string): void {
		...
		const webPartData = ClientSidePage.escapedStringToJson<ClientSideWebpartData>(getAttrValueFromString(
			html,
			'data-sp-webpartdata'
		) as string);
		...
		const canvasDataVersion = getAttrValueFromString(html, 'data-sp-canvasdataversion');
		this.canvasDataVersion = canvasDataVersion && canvasDataVersion.replace(/\\\./, '.');
		const wpDataVersion = getAttrValueFromString(html, 'data-sp-webpartdataversion');
		this.dataVersion = wpDataVersion && wpDataVersion.replace(/\\\./, '.');
	...
	public getControlData(): ClientSideControlData {
		return {
			...
			position: {
				controlIndex: this.order,
				sectionFactor: (this.column && this.column.factor) as CanvasColumnFactorType,
				sectionIndex: (this.column && this.column.order) as number,
				zoneIndex: (this.column && this.column.section && this.column.section.order) as number
			},
	...
	protected renderHtmlProperties(): string {
		...
		if (typeof this.serverProcessedContent === 'undefined' || this.serverProcessedContent === null) {
			...
			if (typeof this.serverProcessedContent.imageSources !== 'undefined') {
				...
				for (let i = 0; i < keys.length; i++) {
					html.push(
						`<img data-sp-prop-name="${keys[i]}" src="${this.serverProcessedContent.imageSources[
							keys[i]
						]}" />`
					);
				}
			...
			if (typeof this.serverProcessedContent.links !== 'undefined') {
				...
				for (let i = 0; i < keys.length; i++) {
					html.push(
						`<a data-sp-prop-name="${keys[i]}" href="${this.serverProcessedContent.links[keys[i]]}"></a>`
					);
				}
			}
		}
	...
	protected parseJsonProperties(props: TypedHash<any>): any {
		...
		if (
			typeof props.webPartData !== 'undefined' &&
			typeof props.webPartData.serverProcessedContent !== 'undefined'
		) {
