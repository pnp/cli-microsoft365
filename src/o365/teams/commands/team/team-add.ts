...
class TeamsTeamAddCommand extends GraphCommand {
  ...
  private waitUntilFinished(operationUrl: string, resolve: () => void, reject: (error: any) => void, cmd: CommandInstance, dots?: string, timeout?: NodeJS.Timer): void {
    ...
    request
      .get(requestOptions)
      .then((res: any) => {
        const response = res as TeamsAsyncOperation;
  ...
  public options(): CommandOption[] {
    const options: CommandOption[] = [
      ...
      },
      {
        ...
        description: 'Local path to the file containing the template. If name or description are supplied, these take prescendence over the template values'
      },
      {
        option: '--wait',
