...
const command: Command = require('./site-office365group-set');
import * as assert from 'assert';
import * as request from 'request-promise-native';
import Utils from '../../../../Utils';

describe(commands.SITE_OFFICE365GROUP_SET, () => {
  let vorpal: Vorpal;
  let log: string[];
  let cmdInstance: any;
  ...
  it('has correct name', () => {
    assert.equal(command.name.startsWith(commands.SITE_OFFICE365GROUP_SET), true);
  });
  ...
  it('logs correct telemetry event', (done) => {
    ...
    cmdInstance.action({ options: {} }, () => {
      try {
        assert.equal(telemetry.name, commands.SITE_OFFICE365GROUP_SET);
        done();
      }
      catch (e) {
        done(e);
  ...
  it('handles error when a group with the specified alias already exists', (done) => {
    ...
    cmdInstance.action({ options: { debug: false, siteUrl: 'https://contoso.sharepoint.com/sites/team-a', alias: 'team-a', displayName: 'Team A' } }, () => {
      try {
        assert(cmdInstanceLogSpy.calledWith(new CommandError('The group alias already exists.')));
        done();
      }
      catch (e) {
        done(e);
  ...
  it('handles error when the specified site already is connected to a group', (done) => {
    ...
    cmdInstance.action({ options: { debug: false, siteUrl: 'https://contoso.sharepoint.com/sites/team-a', alias: 'team-a', displayName: 'Team A' } }, () => {
      try {
        assert(cmdInstanceLogSpy.calledWith(new CommandError('This site already has an O365 Group attached.')));
        done();
      }
      catch (e) {
        done(e);
  ...
  it('has help referring to the right command', () => {
    ...
    cmd.help({}, () => { });
    assert(find.calledWith(commands.SITE_OFFICE365GROUP_SET));
  });
