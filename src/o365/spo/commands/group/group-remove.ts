...
class SpoGroupRemoveCommand extends SpoCommand {
  ...
  public get description(): string {
    return 'Removes site group from specific web';
  }
  ...
  public commandAction(cmd: CommandInstance, args: CommandArgs, cb: () => void): void {
    const removeGroup: () => void = (): void => {
      if (this.verbose) {
        cmd.log(`Removing group in site at ${args.options.webUrl}...`);
      }
  ...
  public options(): CommandOption[] {
    const options: CommandOption[] = [
      ...
      {
        option: '--id [id]',
        description: 'Id of the site group to remove'
      },
      {
        option: '--name [name]',
        description: 'Name of the site group to remove'
      },
      {
        option: '--confirm',
        description: 'Confirm removal of group from site'
      }
    ];
  ...
  Examples:
    ...
    Removes group with id ${chalk.grey('5')} for web ${chalk.grey('https://contoso.sharepoint.com/sites/mysite')}
      ${commands.GROUP_REMOVE} --webUrl https://contoso.sharepoint.com/sites/mysite --id 5

    Removes group with name ${chalk.grey('Team Site Owners')} for web ${chalk.grey('https://contoso.sharepoint.com/sites/mysite')}
