...
describe(commands.ENGAGE_COMMUNITY_GET, () => {
  ...
  it('defines correct alias', () => {
    ...
    assert.deepStrictEqual(alias, [yammerCommands.COMMUNITY_GET]);
  });

  it('correctly logs deprecation warning for yammer command', async () => {
    const chalk = (await import('chalk')).default;
    const loggerErrSpy = sinon.spy(logger, 'logToStderr');
    const commandNameStub = sinon.stub(cli, 'currentCommandName').value(yammerCommands.COMMUNITY_GET);
    sinon.stub(request, 'get').resolves([]);

    await command.action(logger, { options: {} });
    assert.deepStrictEqual(loggerErrSpy.firstCall.firstArg, chalk.yellow(`Command '${yammerCommands.COMMUNITY_GET}' is deprecated. Please use '${commands.ENGAGE_COMMUNITY_GET}' instead.`));

    sinonUtil.restore([loggerErrSpy, commandNameStub]);
  });

  it('correctly handles error 1', async () => {
