...
class SpoReportSiteusageStorageCommand extends PeriodBasedReport {
  ...
  public commandHelp(args: {}, log: (help: string) => void): void {
    ...
    and exports the report data in the specified path in text format
      ${commands.REPORT_SITEUSAGESTORAGE} --period D7 --output text --outputFile 'C:/report.txt'

    Gets the trend of storage allocated and consumed during the last week
    and exports the report data in the specified path in json format
      ${commands.REPORT_SITEUSAGESTORAGE} --period D7 --output json --outputFile 'C:/report.json'
`);
  }
}
