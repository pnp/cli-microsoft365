...
describe(commands.CHAT_MEMBER_REMOVE, () => {
  ...
  it('throws error when member by specifying userName is not found in the chat', async () => {
    ...
    await assert.rejects(command.action(logger, { options: { chatId: chatId, userName: userPrincipalName, confirm: true, verbose: true } }), new CommandError(`Member with userName ${userPrincipalName} could not be found in the chat`));
  });
  ...
  it('throws error when member by specifying userId is not found in the chat', async () => {
    ...
    await assert.rejects(command.action(logger, { options: { chatId: chatId, userId: userId, confirm: true, verbose: true } }), new CommandError(`Member with userId ${userId} could not be found in the chat`));
  });
