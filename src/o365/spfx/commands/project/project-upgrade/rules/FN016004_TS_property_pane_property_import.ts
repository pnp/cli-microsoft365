    ...
    project.tsFiles.forEach(file => {
      ...
      obj.forEach(n => {
        ...
        if (importsToBeMoved.length > 0) {
          ...
          const newBaseImportDeclaration: string = `import { ${importsToStay.toString().replace(/[\[\]']+/g, '')} } from "@microsoft/sp-webpart-base";`;
          const newPropertiesImportDeclaration: string = `import { ${importsToBeMoved.toString().replace(/[\[\]']+/g, '')} } from "@microsoft/sp-property-pane";`;
