...
const command: Command = require('./listitem-record-isrecord');
import * as assert from 'assert';
import request from '../../../../request';
import Utils from '../../../../Utils';

describe(commands.LISTITEM_RECORD_ISRECORD, () => {
  let vorpal: Vorpal;
  let log: any[];
  let cmdInstance: any;
  ...
  it('logs correct telemetry event', (done) => {
    ...
    cmdInstance.action({ options: { debug: true } }, () => {
      try {
        assert.equal(telemetry.name, commands.LISTITEM_RECORD_ISRECORD);
        done();
      }
      catch (e) {
        done(e);
  ...
  it('test a record with list title passed in as an option', (done) => {
    ...
    cmdInstance.action({ options: options }, () => {
      try {
        ...
        assert(cmdInstanceLogSpy.calledWith(vorpal.chalk.green('Completed IsRecord call completed successfully')));
        done();
      }
      catch (e) {
        done(e);
  ...
  it('has help referring to the right command', () => {
    ...
    cmd.help({}, () => { });
    assert(find.calledWith(commands.LISTITEM_RECORD_ISRECORD));
  });
