...
describe(commands.CONTENTTYPEHUB_GET, () => {
  ...
  it('should send correct request body', (done) => {
    ...
    cmdInstance.action({ options: options }, () => {
      try {
        const bodyPayload = `<Request xmlns="http://schemas.microsoft.com/sharepoint/clientquery/2009" AddExpandoFieldTypeSuffix="true" SchemaVersion="15.0.0.0" LibraryVersion="16.0.0.0" ApplicationName="SharePoint Framework CLI v2.0.0">
  <Actions>
  ...
  it('throws error when trying to get contenttype hub URL when not logged in to O365', (done) => {
    auth.service.connected = false;
