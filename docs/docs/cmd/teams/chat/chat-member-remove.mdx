...
`--userId [userId]`
: The Microsoft Entra ID ID of the user. Specify either `id`, `userId`, or `userName` but not multiple.
...
Remove a member from a Teams chat by its Microsoft Entra ID ID.
...
Remove a member from a Teams chat by its Microsoft Entra ID UPN.
