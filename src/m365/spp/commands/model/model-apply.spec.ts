...
describe(commands.MODEL_APPLY, () => {
  ...
  it('apply model to document library by id and list id', async () => {
    sinon.stub(spp, 'getModel').resolves(modelResponse);
  ...
  it('apply model to document library by id and list id to subsite', async () => {
    sinon.stub(spp, 'getModel').resolves(modelResponse);
    sinon.stub(request, 'get').callsFake(async (opts) => {
  ...
  it('correctly handles error when model is not found by its id', async () => {
    sinon.stub(spp, 'getModel').rejects({
      error: {
        "odata.error": {
  ...
  it('correctly handles error when apply model failed', async () => {
    sinon.stub(spp, 'getModel').resolves(modelResponse);
    sinon.stub(request, 'get').callsFake(async (opts) => {
      if (opts.url === `https://contoso.sharepoint.com/sites/sales/_api/web/lists(guid'421b1e42-794b-4c71-93ac-5ed92488b67d')?$select=BaseType,RootFolder/ServerRelativeUrl&$expand=RootFolder`) {
        return listResponse;
