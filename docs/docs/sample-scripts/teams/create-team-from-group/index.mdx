...
<Tabs>
  ...
  $group = $null
  $group = m365 aad m365group add --displayName $displayName `
  ...
  m365 aad m365group set --id $group.id --logoPath $logoPath    
  ...
  $membersToAdd | ForEach-Object {
      ...
      $variable = m365 aad m365group user add --groupId $group.id --userName $_.id -o "json" | convertfrom-json
  }

  Write-Host "Teamify the Group"
  m365 aad m365group teamify --id $group.id
