...
class SpoListComplianceLabelGetCommand extends SpoCommand {
  public get name(): string {
    return commands.LIST_LABEL_GET;
  }

  public get description(): string {
    return 'Gets compliance label set on the specified list';
  }
    ...
    auth
      ...
      .then((accessToken: string): request.RequestPromise => {
        ...
        if (this.verbose) {
          ...
          cmd.log(`Getting compliance label set on the list ${list} in site at ${args.options.webUrl}...`);
        }
      ...
      .then((listInstance: ListInstance): request.RequestPromise => {
        ...
        const requestUrl = `${args.options.webUrl}/_api/SP_CompliancePolicy_SPPolicyStoreProxy_GetListComplianceTag`;
        const requestOptions: any = {
          url: requestUrl,
          headers: Utils.getRequestHeaders({
      ...
      .then((res: any): void => {
        ...
        if (res['odata.null'] === true) {
          cb(new CommandError(`An error has occurred, compliance label could not be retrieved from list '${args.options.listId || args.options.listTitle}'`));
          return;
        }       
        cmd.log(res); 
    ...
    const options: CommandOption[] = [
      {
        ...
        description: 'URL of the site where the list to get the compliance label from is located'
      },
      {
        option: '-l, --listId [listId]',
        description: 'ID of the list to get the compliance label from. Specify either listId or listTitle but not both'
      },
      {
        option: '-t, --listTitle [listTitle]',
        description: 'Title of the list to get the compliance label from. Specify either listId or listTitle but not both'
      }
    ];
...
module.exports = new SpoListComplianceLabelGetCommand();