...
class SpoListLabelSetCommand extends SpoCommand {
  ...
  Remarks:
    ...
    To set a list classification label, you have to first log in to SharePoint using the
    ${chalk.blue(commands.LOGIN)} command, eg. ${chalk.grey(`${config.delimiter} ${commands.LOGIN} https://contoso.sharepoint.com`)}.
        
  Examples:
  
    Sets classification label "Confidential" for list ${chalk.grey('Shared Documents')} located in site
    ...
    Sets classification label "Confidential" and applies 'blockEdit','blockDelete' and 'syncToItems' flags
    for list ${chalk.grey('Shared Documents')} located in site ${chalk.grey('https://contoso.sharepoint.com/sites/project-x')}
  ...
  More information:
    ...
    PnP PowerShell alternative:
      https://docs.microsoft.com/en-us/powershell/module/sharepoint-pnp/set-pnplabel?view=sharepoint-ps
      `);
  }
}
