...
class SpoOrgNewsSiteSetCommand extends SpoCommand {
  ...
  public get description(): string {
    return 'Marks a site as one of multiple possible organizational news sites for the tenant';
  }
  ...
  public options(): CommandOption[] {
    const options: CommandOption[] = [
      {
        option: '-u, --url <url>',
        description: 'Absolute URL of the site to add'
      }
    ];
  ...
  To add a site from the list of organizational news sites, you have to first log in to a tenant admin site using the
    ...
    Using the ${chalk.blue('-u, --url')} option you can specify which site to add to the list of organizational news sites.

  Examples:
  
    Set a site as an organizational news site
      ${chalk.grey(config.delimiter)} ${commands.ORGNEWSSITE_SET} -u https://contoso.sharepoint.com/sites/site1
  `);
  }
}
