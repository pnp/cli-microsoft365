...
class SpoPageAddCommand extends SpoCommand {
  ...
  public commandAction(logger: Logger, args: CommandArgs, cb: () => void): void {
    ...
    let pageTitle: string = args.options.title ? args.options.title : (args.options.name.indexOf('.aspx') > -1 ? args.options.name.substr(0, args.options.name.indexOf('.aspx')) : args.options.name);
