...
In scenarios where you need to set environment variables, for example, you want to use a custom Microsoft Entra ID identity when logging into your Microsoft 365 tenant using the CLI. You can set these variables by passing them in as options arguments `(-e)` into the `docker run` command.
