...
The device code flow is the recommended approach for command-line tools to authenticate with resources secured with Microsoft Entra ID. Because the authentication process is handled in the browser by Microsoft Entra ID itself, it allows you to benefit of rich security features such as multi-factor authentication or conditional access. The device code flow is interactive and requires user interaction which might be limiting if you want to use the CLI for Microsoft 365 in your continuous deployment setup which is fully automated and doesn't involve user interaction.
...
Because there is no user context when logging in using a certificate, you will typically create a new Microsoft Entra ID application, specific to your organization and grant it the required permissions.
...
Because there is no user context when logging in using a secret, you will typically create a new Microsoft Entra ID application, specific to your organization and grant it the required permissions.
...
Currently, SharePoint does not support authentication using Microsoft Entra ID App ID and Secret. CLI for Microsoft 365 commands that call the SharePoint APIs will fail while logged in to Microsoft 365 using a Secret.
...
If you're logged in to Microsoft 365 using a certificate, the `status` command will show the name of the Microsoft Entra ID application used to log in.
...
## Re-consent the PnP Microsoft 365 Management Shell Microsoft Entra ID application

CLI for Microsoft 365 uses the _PnP Microsoft 365 Management Shell_ Microsoft Entra ID application to log in to your Microsoft 365 tenant on your behalf. As we add new commands to the CLI, it's possible, that new permissions will be added to the _PnP Microsoft 365 Management Shell_ Microsoft Entra ID application. To be able to use the newly added commands which depend on these new permissions, you will have to re-approve the _PnP Microsoft 365 Management Shell_ Microsoft Entra ID application in your Microsoft Entra ID. This process is also known as _re-consenting the Microsoft Entra ID application_.

To re-consent the _PnP Microsoft 365 Management Shell_ application in your Microsoft Entra ID, in the command line execute:

```sh
m365 cli reconsent
```

CLI for Microsoft 365 will provide you with a URL that you should open in the web browser and sign in with your organizational account. After authenticating, Microsoft Entra ID will prompt you to approve the new set of permissions. Once you approved the permissions, you will be redirected to an empty page. You can now use all commands in the CLI for Microsoft 365.
