...
Telemetry is sent to Google Analytics. The CLI stores a randomly generated client ID in its local configuration so that events from the same CLI installation can be grouped. This ID isn't derived from personal or device information.
...
- Random client ID stored in the local CLI configuration
...
Docker container | 
CI/CD setup | false
Client ID | Randomly generated identifier for this CLI installation
