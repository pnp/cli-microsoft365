...
import command from './list-defaultcolumnvalue-list.js';
import { z } from 'zod';
import { cli } from '../../../../cli/cli.js';
import { formatting } from '../../../../utils/formatting.js';
import { CommandError } from '../../../../Command.js';

describe(commands.LIST_DEFAULTCOLUMNVALUE_LIST, () => {
  ...
  const defaultColumnValues = [
    {
      Field: 'Countries',
      Value: '19;#Belgium|442affc2-7fab-4f33-9590-330403a579c2;#18;#Croatia|59f1ab85-235b-4cf8-b669-4373cc9393c6',
    ...
    {
      Field: 'Countries',
      Value: '20;#Canada|e3d25461-68ef-4070-8523-5ba439f6d4d5',
      Path: '/sites/Marketing/Shared Documents/Logos'
    },
    {
      Field: 'DocumentType',
      Value: 'Logo',
    ...
    },
    {
      Field: 'DocumentType',
      Value: 'Template',
      Path: '/sites/Marketing/Shared Documents/Templates'
    }
  ];
  ...
  it('has correct name', () => {
    assert.strictEqual(command.name, commands.LIST_DEFAULTCOLUMNVALUE_LIST);
  });
  ...
  it('only outputs one single result', async () => {
    sinon.stub(request, 'get').callsFake(async (opts) => {
      if (opts.url === `${siteUrl}/_api/Web/Lists('${listId}')/RootFolder?$select=ServerRelativeUrl`) {
  ...
  it('correctly retrieves column default values for the specified list by listTitle', async () => {
    sinon.stub(request, 'get').callsFake(async (opts) => {
      if (opts.url === `${siteUrl}/_api/Web/Lists/GetByTitle('${listTitle}')/RootFolder?$select=ServerRelativeUrl`) {
        return { ServerRelativeUrl: listUrl };
      }
  ...
  it('correctly retrieves column default values for the specified list by listUrl', async () => {
    sinon.stub(request, 'get').callsFake(async (opts) => {
      if (opts.url === `${siteUrl}/_api/Web/GetList('${listUrl}')/RootFolder?$select=ServerRelativeUrl`) {
        return { ServerRelativeUrl: listUrl };
      }
  ...
  it('correctly filters column default values for the specified folder', async () => {
    ...
    assert.deepStrictEqual(loggerLogSpy.firstCall.args[0], defaultColumnValues.filter(d => d.Path.toLowerCase() === '/sites/marketing/shared documents/logos'));
  });
  ...
  it('correctly filters column default values for the specified folder with a site relative URL', async () => {
    ...
    assert.deepStrictEqual(loggerLogSpy.firstCall.args[0], defaultColumnValues.filter(d => d.Path.toLowerCase() === '/sites/marketing/shared documents/templates'));
  });
