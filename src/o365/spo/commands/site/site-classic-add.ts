...
class SpoSiteClassicAddCommand extends SpoCommand {
  private currentContext: FormDigestInfo = {} as FormDigestInfo;
      ...
      this
        ...
        .then((res: string): void => {
          ...
          if (response.ErrorInfo) {
            ...
            setTimeout(() => {
              this.waitUntilFinished(JSON.stringify(operation._ObjectIdentity_), resolve, reject, accessToken, cmd, this.currentContext, this.dots, this.timeout);
            }, operation.PollingInterval);
          }
        });
    });
