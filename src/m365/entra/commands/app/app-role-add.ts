...
class EntraAppRoleAddCommand extends GraphCommand {
  ...
  private async getAppObjectId(args: CommandArgs, logger: Logger): Promise<string> {
    ...
    const result = await cli.handleMultipleResultsFound<{ id: string }>(`Multiple Microsoft Entra application registration with name '${appName}' found.`, resultAsKeyValuePair);
    return result.id;
  }
}
