...
class SpoGroupUserRemoveCommand extends SpoCommand {
  ...
  public get description(): string {
    return 'Removes the specified user from specified SharePoint Group';
  }
  ...
  public commandAction(logger: Logger, args: CommandArgs, cb: (err?: any) => void): void {
    const removeUserfromSPGroup = (): void => {
      ...
      const requestUrl: string = `${args.options.webUrl}/_api/web/sitegroups/${args.options.groupId
        ...
        : `GetByName('${encodeURIComponent(args.options.groupName as string)}')`}/users/removeByLoginName(@LoginName)?@LoginName='${encodeURIComponent(loginName as string)}'`;
