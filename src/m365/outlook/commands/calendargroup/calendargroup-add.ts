...
class OutlookCalendarGroupAddCommand extends GraphCommand {
  ...
  public get description(): string {
    return 'Creates a new calendar group for a user';
  }
  ...
  public async commandAction(logger: Logger, args: CommandArgs): Promise<void> {
    try {
      ...
      if (isAppOnlyAccessToken) {
        ...
        if (this.verbose) {
          await logger.logToStderr(`Creating calendar group using application permissions for user '${graphUserId}'...`);
        }
      }
        ...
        if (isOtherUser) {
          ...
          if (!hasSharedScope) {
            throw `To create calendar groups of other users, the Entra ID application used for authentication must have the Calendars.ReadWrite.Shared delegated permission assigned.`;
          }
        }
        ...
        if (this.verbose) {
          await logger.logToStderr(`Creating calendar group using delegated permissions for user '${graphUserId}'...`);
        }
      }
      else {
        ...
        if (this.verbose) {
          await logger.logToStderr('Creating calendar group for the signed-in user...');
        }
      }
