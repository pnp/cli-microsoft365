...
class SpoCustomActionListCommand extends SpoCommand {
  ...
  public commandAction(cmd: CommandInstance, args: CommandArgs, cb: () => void): void {
    ...
    auth
      ...
      .then((contextResponse: ContextInfo): Promise<CustomAction[]> => {
        if (this.debug) {
          cmd.log('Response:');
          cmd.log(JSON.stringify(contextResponse, null, 2));
          cmd.log('');
        ...
        if (args.options.scope && args.options.scope.toLowerCase() !== "all") {
          return this.getCustomActions(args.options, cmd);
        }
  ...
  private getCustomActions(options: Options, cmd: CommandInstance): Promise<CustomAction[]> {
    ...
    if (this.debug) {
      ...
      cmd.log(JSON.stringify(requestOptions, null, 2));
      cmd.log('');
    }
  ...
  private searchAllScopes(options: Options, cmd: CommandInstance): Promise<CustomAction[]> {
    return new Promise<CustomAction[]>((resolve: (list: CustomAction[]) => void, reject: (error: any) => void): void => {
      ...
      this
        .getCustomActions(options, cmd)
        .then((webCustomActions: CustomAction[]): void => {
          if (this.debug) {
            cmd.log('getCustomActions with scope of web. Result...');
            cmd.log(JSON.stringify(webCustomActions, null, 2));
            cmd.log('');
          }
          ...
          this
            ...
            .then((siteCustomActions: CustomAction[]): void => {
              if (this.debug) {
                ...
                cmd.log(JSON.stringify(siteCustomActions, null, 2));
                cmd.log('');
              }

              return resolve(siteCustomActions.concat(webCustomActions));
            }, (err: any): void => {
              reject(err);
            });
        }, (err: any): void => {
