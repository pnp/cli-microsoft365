...
class SpoReportSiteusagePagesCommand extends PeriodBasedReport {
  ...
  public commandHelp(args: {}, log: (help: string) => void): void {
    ...
    and exports the report data in the specified path in text format
      ${commands.REPORT_SITEUSAGEPAGES} --period D7 --output text --outputFile 'C:/report.txt'

    Gets the number of pages viewed across all sites for the last week
    and exports the report data in the specified path in json format
      ${commands.REPORT_SITEUSAGEPAGES} --period D7 --output json --outputFile 'C:/report.json'
`);
  }
}
