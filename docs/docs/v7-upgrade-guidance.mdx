...
In the previous version of CLI for Microsoft 365, you could get deleted groups in Azure AD using the command `aad o365group list` and providing an option `deleted`. In the latest release, we have removed that option from the command. To get all deleted groups in your tenant, you can now use the command [aad o365group recyclebinitem list](./cmd/aad/o365group/o365group-recyclebinitem-list.mdx).

### What action do I need to take?

Remove the references to the `deleted` option in your scripts. Verify that your scripts are using the command [aad o365group recyclebinitem list](./cmd/aad/o365group/o365group-recyclebinitem-list.mdx) instead.
