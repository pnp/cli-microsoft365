...
describe(commands.GROUP_MEMBER_ADD, () => {
  ...
  it('passes validation when all required parameters are valid with group names', async () => {
    const actual = await command.validate({ options: { groupName: 'IT department', subgroupNames: groupNames.join(','), role: 'Member' } }, commandInfo);
    assert.strictEqual(actual, true);
  });
  ...
  it('passes validation when all required parameters are valid with group names with trailing spaces', async () => {
    const actual = await command.validate({ options: { groupName: 'IT department', subgroupNames: groupNames.map(u => u + ' ').join(','), role: 'Member' } }, commandInfo);
    assert.strictEqual(actual, true);
  });
