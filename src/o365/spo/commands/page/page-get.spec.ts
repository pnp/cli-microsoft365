...
describe(commands.PAGE_GET, () => {
  ...
  it('gets information about a modern page on root of tenant', (done) => {
    sinon.stub(request, 'get').callsFake((opts) => {
      if (opts.url.indexOf(`/_api/web/getfilebyserverrelativeurl('/SitePages/home.aspx')`) > -1) {
        return Promise.resolve({
          "ListItemAllFields": {
            "CommentsDisabled": false,
            "FileSystemObjectType": 0,
    ...
    cmdInstance.action({ options: { debug: false, webUrl: 'https://contoso.sharepoint.com', name: 'home.aspx' } }, () => {
      try {
        assert(cmdInstanceLogSpy.calledWith({
          commentsDisabled: false,
          layoutType: 'Home',
