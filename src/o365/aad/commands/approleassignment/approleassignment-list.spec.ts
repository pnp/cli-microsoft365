...
describe(commands.APPROLEASSIGNMENT_LIST, () => {
  ...
  it('retrieves App Role assignments for the specified service principal', (done) => {
    sinon.stub(request, 'get').callsFake((opts) => {
      ...
      if (opts.url.indexOf(`/myorganization/servicePrincipals/b3598f45-9d8c-41c9-b5f0-81eb7ea8551f?api-version=1.6`) > -1) {
        if (opts.headers.accept && opts.headers.accept.indexOf('application/json;odata=nometadata') === 0) {
          return Promise.resolve({
            "objectType": "ServicePrincipal",
    ...
    cmdInstance.action({ options: { output: 'json', appId: '36e3a540-6f25-4483-9542-9f5fa00bb633' } }, () => {
      try {
  ...
  it('correctly handles no app role assignments for the specified app', (done) => {
    sinon.stub(request, 'get').callsFake((opts) => {
      if (opts.url.indexOf(`/myorganization/servicePrincipals?api-version=1.6&$filter=appId eq '141f7648-0c71-4752-9cdb-c7d5305b7e68'`) > -1) {
        if (opts.headers.accept && opts.headers.accept.indexOf('application/json;odata=nometadata') === 0) {
          return Promise.resolve({
            value: []
          });
    ...
    cmdInstance.action({ options: { debug: false, appId: '141f7648-0c71-4752-9cdb-c7d5305b7e68' } }, () => {
      try {
  ...
  it('fails validation if both appId and displayName are specified', () => {
    const actual = (command.validate() as CommandValidate)({ options: { appId: '36e3a540-6f25-4483-9542-9f5fa00bb633', displayName: 'Product Catalog API' } });
    assert.notEqual(actual, true);
  })

  it('passes validation when the appId option specified', () => {
    const actual = (command.validate() as CommandValidate)({ options: { appId: '36e3a540-6f25-4483-9542-9f5fa00bb633' } });
  ...
  it('supports specifying app name', () => {
    const options = (command.options() as CommandOption[]);
    let containsOption = false;
    options.forEach(o => {
