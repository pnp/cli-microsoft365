...
describe(commands.FOLDER_MOVE, () => {
  ...
  it('fails validation if the webUrl option is not a valid SharePoint site URL', async () => {
    const actual = await command.validate({ options: { webUrl: 'invalid', sourceUrl: sourceServerRelUrl, targetUrl: destServerRelUrl } }, commandInfo);
  ...
  it('correctly outputs exactly one result when folder is moved when using sourceId', async () => {
  ...
  it('correctly outputs result when folder is moved when using sourceId', async () => {
    sinon.stub(request, 'get').callsFake(async (opts) => {
      if (opts.url === `${sourceWebUrl}/_api/Web/GetFolderById('${sourceFolderId}')/ServerRelativePath`) {
        return {
  ...
  it('correctly outputs result when folder is moved when using sourceUrl', async () => {
    sinon.stub(request, 'get').callsFake(async (opts) => {
      if (opts.url === `${destWebUrl}/_api/Web/GetFolderById('${destFolderId}')`) {
        return destFolderResponse;
      }
  ...
  it('correctly moves a folder when using sourceId', async () => {
    sinon.stub(request, 'get').callsFake(async (opts) => {
      if (opts.url === `${sourceWebUrl}/_api/Web/GetFolderById('${sourceFolderId}')/ServerRelativePath`) {
        return {
          DecodedUrl: sourceAbsoluteUrl
  ...
  it('correctly moves a folder when using sourceUrl', async () => {
    sinon.stub(request, 'get').callsFake(async (opts) => {
      if (opts.url === `${destWebUrl}/_api/Web/GetFolderById('${destFolderId}')`) {
        return destFolderResponse;
      }
  ...
  it('correctly moves a folder when using site-relative sourceUrl', async () => {
    sinon.stub(request, 'get').callsFake(async (opts) => {
      if (opts.url === `${destWebUrl}/_api/Web/GetFolderById('${destFolderId}')`) {
        return destFolderResponse;
      }
  ...
  it('correctly moves a folder when using absolute urls', async () => {
    sinon.stub(request, 'get').callsFake(async (opts) => {
      if (opts.url === `${destWebUrl}/_api/Web/GetFolderById('${destFolderId}')`) {
        return destFolderResponse;
      }
  ...
  it('correctly moves a folder when using sourceUrl with extra options', async () => {
    sinon.stub(request, 'get').callsFake(async (opts) => {
      if (opts.url === `${destWebUrl}/_api/Web/GetFolderById('${destFolderId}')`) {
        return destFolderResponse;
      }
