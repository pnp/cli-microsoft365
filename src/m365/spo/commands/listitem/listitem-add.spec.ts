...
describe(commands.LISTITEM_ADD, () => {
  ...
  it('ignores global options when creating request data', async () => {
    const addListItemStub = sinon.stub(spoListItem, 'addListItem').resolves();
    ...
    assert.deepEqual(addListItemStub.firstCall.args[0], {
      webUrl: webUrl,
      listId: undefined,
      listUrl: undefined,
      listTitle: 'Demo List',
      folder: 'InsideFolder2/Folder3/',
      contentType: 'Some Content Type',
      fieldValues: { Title: 'Some Title' }
    });
  });

  it('successfully creates a list item', async () => {
    ...
    sinon.stub(spoListItem, 'addListItem').resolves(createdListItem);
    ...
    assert(loggerLogSpy.calledWith(createdListItem));
  });

  it('correctly handles random API error', async () => {
    sinon.stub(spoListItem, 'addListItem').callsFake(() => Promise.reject('An error has occurred'));
