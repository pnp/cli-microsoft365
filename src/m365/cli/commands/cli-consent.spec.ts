...
describe(commands.CONSENT, () => {
  ...
  it('shows consent URL for Viva Engage permissions for the default multi-tenant app', async () => {
    await command.action(logger, { options: { service: 'viva engage' } });
    assert(loggerLogSpy.calledWith(`To consent permissions for executing viva engage commands, navigate in your web browser to https://login.microsoftonline.com/common/oauth2/v2.0/authorize?client_id=31359c7f-bd7e-475c-86db-fdb8c937548e&response_type=code&scope=https%3A%2F%2Fapi.yammer.com%2Fuser_impersonation`));
  });

  it('shows consent URL for Viva Engage permissions for a custom single-tenant app', async () => {
  ...
  it('passes validation if service is set to "viva engage" ', async () => {
