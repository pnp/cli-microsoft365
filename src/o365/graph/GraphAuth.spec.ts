...
describe('GraphAuth', () => {
  ...
  it('reuses existing token if still valid', (done) => {
    ...
    auth
      .ensureAccessToken('https://graph.microsoft.com', stdout)
      .then((accessToken) => {
        try {
  ...
  it('logs error when persisting connection state fails and running in debug mode', (done) => {
    ...
    sinon.stub(auth as any, 'setServiceConnectionInfo').callsFake(() => Promise.reject('An error has occurred'));
    auth
      .ensureAccessToken('https://graph.microsoft.com', stdout, true)
      .then((accessToken) => {
        try {
  ...
  it('stores connection info for the Graph service', () => {
    const authSetServiceConnectionInfoStub = sinon.stub(auth as any, 'setServiceConnectionInfo').callsFake(() => Promise.resolve());
    const site = new Service('https://graph.microsoft.com');
    auth.storeConnectionInfo();
    try {
      assert(authSetServiceConnectionInfoStub.calledWith('Graph', site));
    }
    catch (e) {
      throw e;
    }
  ...
  it('clears connection info for the Graph service', () => {
    ...
    try {
      assert(authClearServiceConnectionInfoStub.calledWith('Graph'));
    }
    catch (e) {
      throw e;
    }
